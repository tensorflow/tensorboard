...
def prepare_graph_for_ui(graph, limit_attr_size=1024,
    ...
    For now, it supports filtering out attributes that are too large to
    be shown in the graph UI.

    Args:
        graph: The GraphDef proto message.
        limit_attr_size: Maximum allowed size in bytes, before the
            attribute is considered large. Default is 1024 (1KB). Must
            be > 0 or None. If None, there will be no filtering.
        large_attrs_key: The attribute key that will be used for storing
            attributes that are too large. Default is
            '_too_large_attrs'. Must be != None if `limit_attr_size` is
            != None.

    Raises:
        ValueError: If `large_attrs_key is None` while
    ...
    if limit_attr_size is not None:
        for node in graph.node:
            # Go through all the attributes and filter out ones bigger
            # than the limit.
            ...
            for key in keys:
                ...
                if size > limit_attr_size or size < 0:
                    del node.attr[key]
                    # Add the attribute key to the list of "too large"
                    # attributes. This is used in the info card in the
                    # graph UI to show the user that some attributes are
