...
class EventMultiplexer(object):
    ...
    Each `EventAccumulator` is associated with a `run`, which is a
    self-contained TensorFlow execution. The `EventMultiplexer` provides
    methods for extracting information about events from multiple
    `run`s.
    ...
    Example usage for loading a directory where each subdirectory is a
    run:
    ...
    If you would like to watch `/parent/directory/path`, wait for it to
    be created (if necessary) and then periodically pick up new runs,
    use `AutoloadingMultiplexer`
    ...
    def __init__(self,
        ...
        Args:
            run_path_map: Dict `{run: path}` which specifies the name of
                a run, and the path to find the associated events. If it
                is None, then the EventMultiplexer initializes without
                any runs.
            size_guidance: A dictionary mapping from `tagType` to the
                number of items to store for each tag of that type. See
    ...
    def AddRun(self, path, name=None):
        ...
        If a run by that name exists, and we are already watching the
        right path, do nothing. If we are watching a different path,
        replace the event accumulator.

        If `Reload` has been called, it will `Reload` the newly created
        accumulators.

        Args:
            path: Path to the event files (or event directory) for given
                run.
            name: Name of the run to add. If not provided, is set to
                path.
        ...
        with self._accumulators_mutex:
            if name not in self._accumulators or self._paths[name] != path:
                if name in self._paths and self._paths[name] != path:
                    # TODO(dandelionmane): Make it impossible to
                    #     overwrite an old path with a new path (just
                    #     give the new path a distinct name)
                    tf.logging.warning(
    ...
    def AddRunsFromDirectory(self, path, name=None):
        ...
        If path doesn't exist, no-op. This ensures that it is safe to
        call `AddRunsFromDirectory` multiple times, even before the
        directory is made.

        If path is a directory, load event files in the directory (if
        any exist) and recursively call AddRunsFromDirectory on any
        subdirectories. This mean you can call AddRunsFromDirectory at
        the root of a tree of event logs and TensorBoard will load them
        all.

        If the `EventMultiplexer` is already loaded this will cause the
        newly created accumulators to `Reload()`.

        Args:
            path: A string path to a directory to load runs from.
            name: Optionally, what name to apply to the runs. If name is
                provided and the directory contains run subdirectories,
                the name of each subrun is the concatenation of the
                parent name and the subdirectory name. If name is
                provided and the directory contains event files, then a
                run is added called "name" and with the events from the
                path.
    ...
    def Reload(self):
        ...
        # Build a list so we're safe even if the list of accumulators is
        # modified even while we're reloading.
        with self._accumulators_mutex:
        ...
        for name, accumulator in items:
            try:
                ...
                tf.logging.error("Unable to reload accumulator '%s': %s", name,
                                 e)
    ...
    def PluginAssets(self, plugin_name):
        ...
        """
        with self._accumulators_mutex:
            # To avoid nested locks, we construct a copy of the
            # run-accumulator map
    ...
    def FirstEventTimestamp(self, run):
        ...
        This may perform I/O if no events have been loaded yet for the
        run.

        Args:
            run: A string name of the run for which the timestamp is
                retrieved.

        Returns:
            The wall_time of the first event of the run, which will
            typically be seconds since the epoch.

        Raises:
            KeyError: If the run is not found.
            ValueError: If the run has no events loaded and there are no
                events on disk to load.
        """
        accumulator = self._GetAccumulator(run)
    ...
    def Scalars(self, run, tag):
        ...
        Raises:
            KeyError: If the run is not found, or the tag is not
                available for the given run.
    ...
    def HealthPills(self, run, node_name):
        ...
        Args:
            run: A string name of the run for which health pills are
                retrieved.
            node_name: A string name of the node for which health pills
                are retrieved.

        Raises:
            KeyError: If the run is not found, or the node name is not
                available for the given run.
    ...
    def GetOpsWithHealthPills(self, run):
        """Determines which ops have at least 1 health pill event for a given
        run.
    ...
    def RunMetadata(self, run, tag):
        """Get the session.run() metadata associated with a TensorFlow run
        and tag.

        Args:
            run: A string name of a TensorFlow run.
            tag: A string name of the tag associated with a particular
                session.run().
        ...
        Returns:
            The metadata in the form of `RunMetadata` protobuf data
            structure.
        """
        accumulator = self._GetAccumulator(run)
    ...
    def Histograms(self, run, tag):
        ...
        Args:
            run: A string name of the run for which values are
                retrieved.
            tag: A string name of the tag for which values are
                retrieved.
