...
Observations for the field. Observations correspond one-to-one with Events in
an event file but contain less information because they only store what is
...
to the lists of Observations. Different functions are applied depending on
the type of field. For instance, for "scalar" fields, the inspector shows
aggregate statistics. For other fields like "session_log:start", all observed
steps are printed in order to aid debugging.
...
def get_unique_tags(field_to_obs):
    ...
    Returns:
        ...
        observations of the type, maps to an empty list so that we can render
        this to console.
    """
...
def get_out_of_order(list_of_numbers):
    ...
    "out-of-order", which may trigger TensorBoard event discarding logic.

    Args:
        list_of_numbers: A list of numbers.

    Returns:
        A list of tuples in which each tuple are two elements are adjacent,
        but the second element is lower than the first.
    """
...
def generators_from_logdir(logdir):
    ...
    within logdir that contain event files. If only logdir contains event
    files, returns a list of length one.
...
def get_inspection_units(logdir='', event_file='', tag=''):
    ...
    If logdir is given, the number of InspectionUnits should equal the number
    of directories or subdirectories that contain event files.
...
def inspect(logdir='', event_file='', tag=''):
    ...
    Raises:
        ValueError: If neither logdir and event_file are given, or both are
        given.
    """
