...
subdirectory with event files. If running over an event file, it creates
only one InspectionUnit. One block of output is printed to console for
each InspectionUnit.

The primary content of an InspectionUnit is the dict field_to_obs that
maps fields (e.g. "scalar", "histogram", "session_log:start", etc.) to a
list of Observations for the field. Observations correspond one-to-one
with Events in an event file but contain less information because they
only store what is necessary to generate the final console output.

The final output is rendered to console by applying some aggregating
function to the lists of Observations. Different functions are applied
depending on the type of field. For instance, for "scalar" fields, the
inspector shows aggregate statistics. For other fields like
"session_log:start", all observed steps are printed in order to aid
debugging.


[1] Query a logdir or an event file for its logged tags and summary
    statistics using --logdir or --event_file.
...
# Map of field names within summary.proto to the user-facing names that
# this script outputs.
...
# An `Observation` contains the data within each Event file that the
# inspector cares about. The inspector accumulates Observations as it
# processes events.
Observation = collections.namedtuple('Observation', ['step', 'wall_time',
                                                     'tag'])

# An InspectionUnit is created for each organizational structure in the
# event files visible in the final terminal output. For instance, one
# InspectionUnit is created for each subdirectory in logdir. When asked
# to inspect a single event file, there may only be one InspectionUnit.

# The InspectionUnit contains the `name` of the organizational unit that
# will be printed to console, a `generator` that yields `Event` protos,
# and a mapping from string fields to `Observations` that the inspector
# creates.
...
def get_field_to_observations_map(generator, query_for_tag=''):
    ...
    Args:
        ...
        query_for_tag: A string that if specified, only create
            observations for events with this tag name.

    Returns:
        A dict mapping keys in `TRACKED_FIELDS` to an `Observation`
        list.
    """
...
def get_unique_tags(field_to_obs):
    ...
    Returns:
        A dict that maps keys in `TAG_FIELDS` to a list of string tags
        present in the event files. If the dict does not have any
        observations of the type, maps to an empty list so that we can
        render this to console.
    """
...
def get_out_of_order(list_of_numbers):
    ...
    This is used to find instances of global step values that are
    "out-of-order", which may trigger TensorBoard event discarding
    logic.
    ...
    Returns:
        A list of tuples in which each tuple are two elements are
        adjacent, but the second element is lower than the first.
    """
...
def generators_from_logdir(logdir):
    ...
    The number of generators returned should equal the number of
    directories within logdir that contain event files. If only logdir
    contains event files, returns a list of length one.
...
def get_inspection_units(logdir='', event_file='', tag=''):
    ...
    if logdir:
        ...
        if inspection_units:
            ...
            print(
                'It seems that {} may be an event file instead of a logdir. '
                'If this '
...
def inspect(logdir='', event_file='', tag=''):
    ...
    Raises:
        ValueError: If neither logdir and event_file are given, or both
            are given.
    """
    ...
    for unit in inspection_units:
        if tag:
            ...
            # If the user is not inspecting a particular tag, also print the
            # list of
