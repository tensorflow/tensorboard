...
class DbImportMultiplexer(plugin_event_multiplexer.EventMultiplexer):
  ...
  """
    ...
    def __init__(
        self, db_uri, db_connection_provider, purge_orphaned_data, max_reload_threads
    ):
    ...
    """
        ...
        if self._purge_orphaned_data:
            logger.warn("--db_import does not yet support purging orphaned data")
        ...
        for subdir in io_wrapper.GetLogdirSubdirectories(path):
            ...
            if subdir not in self._run_loaders:
                ...
                self._run_loaders[subdir] = _RunLoader(
                    subdir=subdir, experiment_name=experiment_name, run_name=run_name
                )
