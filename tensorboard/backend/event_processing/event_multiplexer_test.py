...
class _FakeAccumulator(object):
    def __init__(self, path, health_pill_mapping=None):
        ...
        Args:
            ...
            health_pill_mapping: An optional mapping from Op to health pill
            strings.
        """
        self._path = path
    ...
    def CompressedHistograms(self, tag_name):
        return self._TagHelper(tag_name,
...
class EventMultiplexerTest(tf.test.TestCase):
    ...
    def testHealthPills(self):
        ...
        x = event_multiplexer.EventMultiplexer(
            {'run1': 'path1', 'run2': 'path2'})
        self.assertEqual(['path1/hp1', 'path1/hp2'],
                         x.HealthPills('run1', 'Add'))
    ...
    def testAddRun(self):
        ...
        x.AddRun('run1_path', 'run1')
        self.assertEqual(run1, x._GetAccumulator('run1'),
                         'loader not recreated')
