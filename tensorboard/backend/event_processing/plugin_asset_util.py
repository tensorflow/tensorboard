...
def ListPlugins(logdir):
    ...
    maintains compatibility with old directories that have no plugins written.
...
def ListAssets(logdir, plugin_name):
    ...
    Returns:
        A string list of available plugin assets. If the plugin subdirectory
        does not exist (either because the logdir doesn't exist, or because
        the plugin didn't register) an empty list is returned.
    """
