...
class ReservoirBucketTest(tf.test.TestCase):
    ...
    def testLazyFunctionEvaluationAndAlwaysKeepLast(self):
        ...
        # We've mocked the randomness generator, so that once it is
        # full, the last item will never get durable reservoir
        # inclusion. Since always_keep_last is false, the function
        # should only get invoked 100 times while filling up the
        # reservoir. This laziness property is an essential performance
        # optimization.
        b = reservoir._ReservoirBucket(100, FakeRandom(),
        ...
        # This time, we will always keep the last item, meaning that the
        # function should get invoked once for every item we add.
...
class ReservoirBucketStatisticalDistributionTest(tf.test.TestCase):
    ...
    def testBucketReservoirSamplingViaStatisticalProperties(self):
        # Not related to a 'ReservoirBucket', but instead number of
        # buckets we put samples into for testing the shape of the
        # distribution
        b = reservoir._ReservoirBucket(_max_size=self.samples)
        # add one extra item because we always keep the most recent
        # item, which would skew the distribution; we can just slice it
        # off the end instead.
