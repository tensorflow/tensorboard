...
class IoWrapperTest(tb_test.TestCase):
    def setUp(self):
        pass
    ...
    def testListDirectoryAbsolute(self):
        ...
        importlib.reload(io_wrapper)
        temp_dir = tempfile.mkdtemp(prefix=self.get_temp_dir())
        self._CreateDeepDirectoryStructure(temp_dir)
        io_wrapper.GetLogdirSubdirectories(
            temp_dir, concurrent_dir_discover=True, cache_TTL=30
        )
        io_wrapper.ListRecursivelyViaWalking(temp_dir)
        expected_files = (
            "foo",
            "bar",
            "quuz",
