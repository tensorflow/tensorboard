...
CompressedHistogramEvent = namedtuple(
    "CompressedHistogramEvent", ["wall_time", "step", "compressed_histogram_values"]
)

HistogramEvent = namedtuple("HistogramEvent", ["wall_time", "step", "histogram_value"])
...
ImageEvent = namedtuple(
    "ImageEvent", ["wall_time", "step", "encoded_image_string", "width", "height"]
)
  ...
  """
                ...
                if value.HasField("metadata"):
                    ...
                    if tag not in self.summary_metadata:
                        ...
                        if plugin_data.plugin_name:
                            self._plugin_to_tag_to_content[plugin_data.plugin_name][
                                tag
                            ] = plugin_data.content
                        else:
                            logger.warn(
    ...
    """
        ...
        if by_tags:
            ...
            expired_per_tags = [_ExpiredPerTag(value) for value in event.summary.value]
...
def _GeneratorFromPath(path):
    ...
    if io_wrapper.IsTensorFlowEventsFile(path):
        ...
        return directory_watcher.DirectoryWatcher(
            path, event_file_loader.EventFileLoader, io_wrapper.IsTensorFlowEventsFile
        )
