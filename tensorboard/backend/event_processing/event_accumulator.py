...
# naturally gives bands around median of width 1 std dev, 2 std dev, 3
# std dev, and then the long tail.
...
# The tag that values containing health pills have. Health pill data is
# stored in tensors. In order to distinguish health pill values from
# scalar values, we rely on how health pill values have this special tag
# value.
...
class EventAccumulator(object):
    ...
    TensorFlow writes out `Event` protobuf objects, which have a
    timestamp and step number, and often contain a `Summary`. Summaries
    can have different kinds of data like an image, a scalar value, or a
    histogram.  The Summaries also have a tag, which we use to organize
    logically related data. The `EventAccumulator` supports retrieving
    the `Event` and `Summary` data by its tag.

    Calling `Tags()` gets a map from `tagType` (e.g. `'images'`,
    `'compressedHistograms'`, `'scalars'`, etc) to the associated tags
    for those data types. Then, various functional endpoints (e.g.
    `Accumulator.Scalars(tag)`) allow for the retrieval of all data
    associated with that tag.

    The `Reload()` method synchronously loads all of the data written so
    far.

    Histograms, audio, and images are very large, so storing all of them
    is not recommended.
    ...
    def __init__(self,
        ...
        Args:
            path: A file path to a directory containing tf events files,
                or a single tf events file. The accumulator will load
            ...
            purge_orphaned_data: Whether to discard any events that were
        ...
        # Unlike the other reservoir, the reservoir for health pills is
        # keyed by the name of the op instead of the tag. This lets us
        # efficiently obtain the health pills per node.
    ...
    def PluginAssets(self, plugin_name):
        ...
        Args:
            plugin_name: The string name of a plugin to retrieve assets
                for.

        Returns:
            A list of string plugin asset names, or empty list if none
            are available. If the plugin was not registered, an empty
            list is returned.
        """
    ...
    def RetrievePluginAsset(self, plugin_name, asset_name):
        ...
        """
        return plugin_asset_util.RetrieveAsset(self.path, plugin_name,
                                               asset_name)

    def FirstEventTimestamp(self):
        """Returns the timestamp in seconds of the first event.

        If the first event has been loaded (either by this method or by
        `Reload`, this returns immediately. Otherwise, it will load in
        the first event.  Note that this means that calling `Reload`
        will cause this to block until `Reload` has finished.

        Returns:
            The timestamp in seconds of the first event that was loaded.

        Raises:
            ValueError: If no events have been loaded and there were no
                events found on disk.
        """
    ...
    def _ProcessEvent(self, event):
        ...
        # Process the event.
        #
        # GraphDef and MetaGraphDef are handled in a special way: If no
        # graph_def Event is available, but a meta_graph_def is, and it
        # contains a graph_def, then use the meta_graph_def.graph_def as
        # our graph. If a graph_def Event is available, always prefer it
        # to the graph_def inside the meta_graph_def.
        if event.HasField('graph_def'):
            ...
            if self._meta_graph is not None:
                tf.logging.warn(('Found more than one metagraph event per run. '
                                 'Overwriting the metagraph with the newest '
                                 'event.'))
                    ...
                    if self._graph is not None:
                        tf.logging.warn(
                            ('Found multiple metagraphs containing graph_defs,'
                             'but did not find any graph events.  Overwriting '
                             'the graph with the newest metagraph version.'))
                ...
                if (value.HasField('tensor') and
                    ...
                    for summary_type, summary_func in SUMMARY_TYPES.items():
                        if value.HasField(summary_type):
                            ...
                            tag = value.node_name if summary_type == 'tensor' \
                                else value.tag
    ...
    def _ProcessHealthPillSummary(self, value, event):
        ...
        These summaries are distinguished by the fact that they have a
        Tensor field and have a special tag value.

        This method emits ERROR-level messages to the logs if it
        encounters Tensor summaries that it cannot process.
        ...
        # The node_name property of the value object is actually a watch
        # key: a combination of node name, output slot, and a suffix. We
        # capture the actual node name and the output slot with a
        # regular expression.
        match = re.match(r'^(.*):(\d+):DebugNumericSummary$', value.node_name)
        if not match:
            tf.logging.log_first_n(
                tf.logging.ERROR,
                'Unsupported watch key %s for health pills; skipping this '
                'sequence.',
                1, value.node_name)
            return
        ...
        self._ProcessHealthPill(event.wall_time, event.step, device_name,
                                node_name,
    ...
    def Graph(self):
        ...
        If the graph is stored directly, return that.  If no graph is
        stored directly but a metagraph is stored containing a graph,
        return that.
    ...
    def _MaybePurgeOrphanedData(self, event):
        ...
        When TensorFlow crashes at step T+O and restarts at step T, any
        events written after step T are now "orphaned" and will be at
        best misleading if they are included in TensorBoard.

        This logic attempts to determine if there is orphaned data, and
        purge it if it is found.

        Args:
            event: The event to use as a reference, to determine if a
                purge is needed.
        """
    ...
    def _CheckForRestartAndMaybePurge(self, event):
        ...
        Check for a SessionLog.START event and purge all previously seen
        events with larger steps, because they are out of date. Because
        of supervisor threading, it is possible that this logic will
        cause the first few event messages to be discarded since
        supervisor threading does not guarantee that the START message
        is deterministically written first.

        This method is preferred over
        _CheckForOutOfOrderStepAndMaybePurge which can inadvertently
        discard events due to supervisor threading.

        Args:
            event: The event to use as reference. If the event is a
                START event, all previously seen events with a greater
                event.step will be purged.
        """
        if event.HasField(
    ...
    def _CheckForOutOfOrderStepAndMaybePurge(self, event):
        ...
        Check if the event is out of order relative to the global most
        recent step. If it is, purge outdated summaries for tags that
        the event contains.

        Args:
            event: The event to use as reference. If the event is
                out-of-order, all events with the same tags, but with a
                greater event.step will be purged.
        """
    ...
    def _ProcessHistogram(self, tag, wall_time, step, histo):
        ...
        self._compressed_histograms.AddItem(tag, histo_ev,
                                            self._CompressHistogram)
    ...
    def _ProcessHealthPill(self, wall_time, step, device_name, node_name,
        ...
        Args:
            wall_time: The time at which the health pill was
                created. Provided by the debugger.
            step: The step at which the health pill was
                created. Provided by the debugger.
            ...
            elements: An ND array of 20 floats. The elements of the
                health pill.
        """
        # Key by the node name for fast retrieval of health pills by
        # node name. The array is cast to a list so that it is
        # JSON-able. The debugger data plugin serves a JSON response.
    ...
    def _Purge(self, event, by_tags):
        ...
        If by_tags is True, purge all events that occurred after the
        given event.step, but only for the tags that the event
        has. Non-sequential event.steps suggest that a TensorFlow
        restart occurred, and we discard the out-of-order events to
        display a consistent view in TensorBoard.

        Discarding by tags is the safer method, when we are unsure
        whether a restart has occurred, given that threading in
        supervisor can cause events of different tags to arrive with
        unsynchronized step values.

        If by_tags is False, then purge all events with event.step
        greater than the given event.step. This can be used when we are
        certain that a TensorFlow restart has occurred and these events
        can be discarded.

        Args:
            event: The event to use as reference for the purge. All
                events with the same tags, but with a greater event.step
                will be purged.
            by_tags: Bool to dictate whether to discard all out-of-order
                events or only those that are associated with the given
                reference event.
        """
