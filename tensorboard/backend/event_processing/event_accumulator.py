...
class EventAccumulator(object):
    ...
    different kinds of data like an image, a scalar value, or a histogram.
    The Summaries also have a tag, which we use to organize logically related
    data. The `EventAccumulator` supports retrieving the `Event` and
    `Summary` data by its tag.

    Calling `Tags()` gets a map from `tagType` (e.g. `'images'`,
    `'compressedHistograms'`, `'scalars'`, etc) to the associated tags for
    those data types. Then, various functional endpoints (eg
    ...
    def __init__(self,
        ...
        Args:
            path: A file path to a directory containing tf events files,
                or a single tf events file. The accumulator will load events
                from this path.
            size_guidance: Information on how much data the EventAccumulator
                should store in memory. The DEFAULT_SIZE_GUIDANCE tries not to
                store too much so as to avoid OOMing the client. The
                size_guidance should be a map from a `tagType` string to an
                integer representing the number of items to keep per tag for
                items of that `tagType`. If the size is 0, all events are
                stored.
            compression_bps: Information on how the `EventAccumulator` should
                compress histogram data for the `CompressedHistograms` tag (for
                details see `ProcessCompressedHistogram`).
        ...
        # Unlike the other reservoir, the reservoir for health pills is keyed
        #  by the
    ...
    def PluginAssets(self, plugin_name):
        ...
        Returns:
            A list of string plugin asset names, or empty list if none are
            available. If the plugin was not registered, an empty list is
            returned.
        """
    ...
    def FirstEventTimestamp(self):
        ...
        `Reload`,
        this returns immediately. Otherwise, it will load in the first event.
        Note
    ...
    def _ProcessEvent(self, event):
        ...
        # contains a graph_def, then use the meta_graph_def.graph_def as our
        # graph. If a graph_def Event is available, always prefer it to the
        # graph_def inside the meta_graph_def.
    ...
    def _ProcessHealthPillSummary(self, value, event):
        ...
        These summaries are distinguished by the fact that they have a Tensor
        field and have a special tag value.

        This method emits ERROR-level messages to the logs if it encounters
        Tensor summaries that it cannot process.
        ...
        # The node_name property of the value object is actually a watch key:
        # a combination of node name, output slot, and a suffix. We capture
        # the actual node name and the output slot with a regular expression.
    ...
    def _MaybePurgeOrphanedData(self, event):
        ...
        When TensorFlow crashes at step T+O and restarts at step T,
        any events written after step T are now "orphaned" and will be at
        ...
        Args:
            event: The event to use as a reference, to determine if a purge is
                needed.
        """
    ...
    def _CheckForRestartAndMaybePurge(self, event):
        ...
        events with larger steps, because they are out of date. Because of
        supervisor threading, it is possible that this logic will cause the
        first few event messages to be discarded since supervisor threading
        does not guarantee that the START message is deterministically
        written first.

        This method is preferred over _CheckForOutOfOrderStepAndMaybePurge
        which can inadvertently discard events due to supervisor threading.

        Args:
            event: The event to use as reference. If the event is a START event,
                all previously seen events with a greater event.step will be
                purged.
        """
        if event.HasField(
    ...
    def _CheckForOutOfOrderStepAndMaybePurge(self, event):
        ...
        Check if the event is out of order relative to the global most recent
        step. If it is, purge outdated summaries for tags that the event
        contains.

        Args:
            event: The event to use as reference. If the event is
                out-of-order, all events with the same tags, but with a greater
    ...
    def _ProcessHealthPill(self, wall_time, step, device_name, node_name,
        ...
        Args:
            wall_time: The time at which the health pill was created. Provided
                by the debugger.
        ...
        """
        # Key by the node name for fast retrieval of health pills by node
        # name. The array is cast to a list so that it is JSON-able. The
        # debugger data plugin serves a JSON response.
    ...
    def _Purge(self, event, by_tags):
        ...
        event.steps suggest that a TensorFlow restart occurred, and we
        discard the out-of-order events to display a consistent view in
        TensorBoard.

        Discarding by tags is the safer method, when we are unsure whether a
        restart has occurred, given that threading in supervisor can cause
        events of different tags to arrive with unsynchronized step values.

        If by_tags is False, then purge all events with event.step greater
        than the given event.step. This can be used when we are certain that
        a TensorFlow restart has occurred and these events can be discarded.
        ...
        Args:
            ...
            by_tags: Bool to dictate whether to discard all out-of-order events
                or only those that are associated with the given reference
                event.
        """
