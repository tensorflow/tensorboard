...
class IoWrapperTest(tf.test.TestCase):
    ...
    def testIsIsTensorFlowEventsFileFalse(self):
        self.assertFalse(io_wrapper.IsTensorFlowEventsFile("/logdir/model.ckpt"))

    def testIsIsTensorFlowEventsFileWithEmptyInput(self):
        with six.assertRaisesRegex(self, ValueError, r"Path must be a nonempty string"):
            io_wrapper.IsTensorFlowEventsFile("")
    ...
    def testListRecursivelyViaGlobbing(self):
        ...
        expected = [
            ...
            ["bar/quux", ["some_flume_output.txt", "some_more_flume_output.txt",]],
    ...
    """
        ...
        gotten_directory_to_listing = {result[0]: list(result[1]) for result in gotten}
        self.assertItemsEqual(
            expected_directory_to_listing.keys(), gotten_directory_to_listing.keys()
        )
