...
class DirectoryWatcherTest(tf.test.TestCase):
    def setUp(self):
        ...
        self._watcher = directory_watcher.DirectoryWatcher(self._directory, _ByteLoader)
    ...
    def testPathFilter(self):
        self._watcher = directory_watcher.DirectoryWatcher(
            self._directory, _ByteLoader, lambda path: "do_not_watch_me" not in path
        )
    ...
    def testDoesntRaiseDirectoryDeletedErrorIfOutageIsTransient(self):
        ...
        for stub_name in stub_names:
            self.stubs.Set(
                io_wrapper, stub_name, FakeFactory(getattr(io_wrapper, stub_name))
            )
        for stub_name in ["exists", "stat"]:
            self.stubs.Set(
                tf.io.gfile, stub_name, FakeFactory(getattr(tf.io.gfile, stub_name))
            )
