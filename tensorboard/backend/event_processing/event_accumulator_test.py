...
class _EventGenerator(object):
    ...
    Satisfies the EventWriter API required to create a
    ...
    def AddImage(self,
        ...
        image = tf.Summary.Image(
            encoded_image_string=encoded_image_string, width=width,
            height=height)
        event = tf.Event(
            wall_time=wall_time,
            step=step,
...
class EventAccumulatorTest(tf.test.TestCase):
    def assertTagsEqual(self, actual, expected):
        ...
        It fills out the `expected` arg with the default (empty) values
        for every tag type, so that the author needs only specify the
        non-empty values they are interested in testing.

        Args:
            actual: The actual Accumulator tags response.
            expected: The expected tags response (empty fields may be
                omitted).
        """
        ...
        # Verifies that there are no unexpected keys in the actual
        # response.  If this line fails, likely you added a new tag
        # type, and need to update the empty_tags dictionary above.
...
class MockingEventAccumulatorTest(EventAccumulatorTest):
    ...
    def testExpiredDataDiscardedAfterRestartForFileVersionLessThan2(self):
        ...
        If a step value is observed to be lower than what was previously
        seen, this should force a discard of all previous items with the
        same tag that are outdated.

        Only file versions < 2 use this out-of-order discard logic.
        Later versions discard events based on the step value of
        SessionLog.START.
        """
        warnings = []
    ...
    def testEventsDiscardedPerTagAfterRestartForFileVersionLessThan2(self):
        """Tests that event discards after restart, only affect the
        misordered tag.

        If a step value is observed to be lower than what was previously
        seen, this should force a discard of all previous items that are
        outdated, but only for the out of order tag. Other tags should
        remain unaffected.
    ...
    def testReloadPopulatesFirstEventTimestamp(self):
        ...
        acc = ea.EventAccumulator(gen)
        gen.AddEvent(
            tf.Event(wall_time=1, step=2, file_version='brain.Event:2'))
    ...
    def testTFSummaryScalar(self):
        ...
        seq1 = [ea.ScalarEvent(wall_time=0, step=i, value=i) for i in
                xrange(10)]
        seq2 = [
            ea.ScalarEvent(
    ...
    def testTFSummaryImage(self):
        ...
        with self.test_session() as sess:
            ...
            # This is an interesting example, because the old
            # tf.image_summary op would throw an error here, because it
            # would be tag reuse.  Using the tf node name instead allows
            # argument re-use to the image summary.
