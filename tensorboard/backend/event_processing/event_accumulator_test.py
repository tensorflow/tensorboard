...
class MockingEventAccumulatorTest(EventAccumulatorTest):
    ...
    def testImages(self):
        ...
        acc = ea.EventAccumulator(gen)
        im1 = ea.ImageEvent(
            wall_time=1, step=10, encoded_image_string=b"big", width=400, height=300
        )
        im2 = ea.ImageEvent(
            wall_time=2, step=12, encoded_image_string=b"small", width=40, height=30
        )
        gen.AddImage(
            "im1",
            wall_time=1,
    ...
    """
        ...
        gen.AddEvent(event_pb2.Event(wall_time=0, step=0, file_version="brain.Event:1"))
    ...
    """
        gen = _EventGenerator(self)
        acc = ea.EventAccumulator(gen)
        gen.AddEvent(event_pb2.Event(wall_time=0, step=1, file_version="brain.Event:2"))
    ...
    def testReloadPopulatesFirstEventTimestamp(self):
        ...
        gen = _EventGenerator(self)
        acc = ea.EventAccumulator(gen)
        gen.AddEvent(event_pb2.Event(wall_time=1, step=2, file_version="brain.Event:2"))
    ...
    def testTFSummaryScalar(self):
        ...
        seq2 = [ea.ScalarEvent(wall_time=0, step=i, value=i * i) for i in xrange(10)]

        self.assertTagsEqual(
            accumulator.Tags(),
            {ea.SCALARS: ["scalar1", "scalar2"], ea.GRAPH: True, ea.META_GRAPH: False,},
        )
    ...
    def testTFSummaryImage(self):
        ...
        self.assertTagsEqual(
            accumulator.Tags(), {ea.IMAGES: tags, ea.GRAPH: True, ea.META_GRAPH: False,}
        )
...
class RealisticEventAccumulatorTest(EventAccumulatorTest):
    ...
    def testSummaryMetadata(self):
        ...
        acc.Reload()
        self.assertProtoEquals(summary_metadata, acc.SummaryMetadata("you_are_it"))
    ...
    def testSummaryMetadata_FirstMetadataWins(self):
        ...
        self.assertProtoEquals(summary_metadata_1, acc.SummaryMetadata("you_are_it"))
    ...
    def testPluginTagToContent_PluginsCannotJumpOnTheBandwagon(self):
        ...
        self.assertEqual(acc.PluginTagToContent("outlet"), {"you_are_it": b"120v"})
