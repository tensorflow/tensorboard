...
class EventAccumulatorTest(tf.test.TestCase):
    def assertTagsEqual(self, actual, expected):
        ...
        It fills out the `expected` arg with the default (empty) values for
        every tag type, so that the author needs only specify the non-empty
        values they are interested in testing.
        ...
        # If this line fails, likely you added a new tag type, and need to
        # update
...
class MockingEventAccumulatorTest(EventAccumulatorTest):
    ...
    def testExpiredDataDiscardedAfterRestartForFileVersionLessThan2(self):
        ...
        seen, this should force a discard of all previous items with the same
        tag that are outdated.

        Only file versions < 2 use this out-of-order discard logic. Later
        versions discard events based on the step value of SessionLog.START.
        """
        warnings = []
    ...
    def testEventsDiscardedPerTagAfterRestartForFileVersionLessThan2(self):
        ...
        outdated, but only for the out of order tag. Other tags should remain
        unaffected.
    ...
    def testSessionLogStartMessageDiscardsExpiredEvents(self):
        ...
        This discard logic is preferred over the out-of-order step discard
        logic, but this logic can only be used for event protos which have
        the SessionLog enum, which was introduced to event.proto for
