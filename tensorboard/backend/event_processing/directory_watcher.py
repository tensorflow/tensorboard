...
class DirectoryWatcher(object):
    ...
    written to by the data source at a time and that once the source stops
    writing to a path, it will start writing to a new path that's
    lexicographically greater and never come back. It uses some heuristics to
    check whether this is true based on tracking changes to the files' sizes,
    but the check can have false negatives. However, it should have no false
    positives.
    """
    ...
    def __init__(self, directory, loader_factory, path_filter=lambda x: True):
        ...
        Args:
            ...
            loader_factory: A factory for creating loaders. The factory
                should take a path and return an object that has a Load method
                returning an iterator that will yield all events that have not
                been yielded yet.
            path_filter: If specified, only paths matching this filter are
            loaded.
    ...
    def Load(self):
        ...
        The watcher will load from one path at a time; as soon as that path
        stops yielding events, it will move on to the next path. We assume
        that old paths are never modified after a newer path has been
        written. As a result, Load() can be called multiple times in a row
        without losing events that have not been yielded yet. In other words,
        we guarantee that every event will be yielded exactly once.
    ...
    def _LoadInternal(self):
        ...
        The only difference between this and Load() is that the latter will
        throw DirectoryDeletedError on I/O errors if it thinks that the
        ...
        while True:
            ...
            if not next_path:
                ...
                # Current path is empty and there are no new paths, so we're
                # done.
                return

            # There's a new path, so check to make sure there weren't any
            # events written between when we finished reading the current
            # path and when we checked for the new one. The sequence of
            # events might look something like this:
            #
            # 1. Event #1 written to path #1.
            # 2. We check for events and yield event #1 from path #1
            # 3. We check for events and see that there are no more events in
            #  path #1.
            ...
            # guaranteed by the loader contract that no more events will be
            # written to path #1 after events start being written to path #2,
    ...
    # The number of paths before the current one to check for out of order
    # writes.
    ...
    def OutOfOrderWritesDetected(self):
        ...
        Out-of-order writes are only checked as part of the Load() iterator.
        Once an out-of-order write is detected, this function will always
        return true.

        Note that out-of-order write detection is not performed on GCS paths,
        so this function will always return false.

        Returns:
            Whether any out-of-order write has ever been detected by this
            watcher.
        """
    ...
    def _GetNextPath(self):
        ...
        This function also does the checking for out-of-order writes as it
        iterates through the paths.

        Returns:
            The next path to load events from, or None if there are no more
            paths.
        """
        paths = sorted(path
                       for path in
        ...
        # Don't bother checking if the paths are GCS (which we can't check)
        # or if
...
class DirectoryDeletedError(Exception):
    ...
    We distinguish this from temporary errors so that other code can decide
    to drop all of our data only when a directory has been intentionally
    deleted, as opposed to due to transient filesystem errors.
    """
    pass
