...
class FakePlugin(base_plugin.TBPlugin):
    ...
    def __init__(self, unused_context, plugin_name, is_active_value,
        ...
        Args:
            ...
            routes_mapping: A dictionary mapping from route (string URL
                path) to the method called when a user issues a request
                to that route.
        """
    ...
    def get_plugin_apps(self):
        ...
        Returns:
            A dictionary mapping from routes to handlers offered by this
            plugin.
        """
...
class TensorboardServerTest(tf.test.TestCase):
    ...
    def setUp(self):
        ...
        plugins = [
            FakePlugin(
                None, plugin_name='foo', is_active_value=True,
                routes_mapping={}),
            FakePlugin(
                None, plugin_name='bar', is_active_value=False,
                routes_mapping={}),
        ]
        ...
        try:
            ...
            # BaseWSGIServer has a preference for IPv4. If that didn't work,
            # try again
    ...
    def _GenerateTestData(self, run_name):
        ...
        The test data has a single run of the given name, containing a
        graph definition and metagraph definition.

        Arguments:
            run_name: The directory under self.logdir into which to
                write events.
        """
    ...
    def _test(self, name, should_be_okay):
        ...
        plugins = [
            FakePlugin(
                ...
                routes_mapping={}),
            FakePlugin(
                None, plugin_name=name, is_active_value=True,
                routes_mapping={}),
            FakePlugin(
                None, plugin_name='bar', is_active_value=False,
                routes_mapping={}),
...
class TensorboardSimpleServerConstructionTest(tf.test.TestCase):
    ...
    Mostly useful for IPv4/IPv6 testing. This test should run with only IPv4,
    only IPv6, and both IPv4 and IPv6 enabled.
    """
...
class TensorBoardApplcationConstructionTest(tf.test.TestCase):
    def testExceptions(self):
        ...
        with self.assertRaises(ValueError):
            ...
            plugins = [
                FakePlugin(
                    None, plugin_name=None, is_active_value=True,
                    routes_mapping={}),
            ]
