...
TensorBoardApplication constructs TensorBoard as a WSGI application. It
handles serving static assets, and implements TensorBoard data APIs.
"""
...
def standard_tensorboard_wsgi(
    ...
    Args:
        ...
        reload_interval: The interval at which the backend reloads more
            data in seconds.
        plugins: A list of constructor functions for TBPlugin
            subclasses.
...
class TensorBoardWSGIApp(object):
    ...
    def __init__(self, logdir, plugins, multiplexer, reload_interval):
        ...
        Args:
            logdir: the logdir spec that describes where data will be
                loaded.  may be a directory, or comma,separated list of
                directories, or colons can be used to provide named
                directories
            plugins: A list of base_plugin.TBPlugin subclass instances.
            multiplexer: The EventMultiplexer with TensorBoard data to
                serve.
            reload_interval: How often (in seconds) to reload the
                Multiplexer.
        ...
        Raises:
            ...
            ValueError: If some plugin has an invalid plugin_name
                (plugin names must only contain [A-Za-z0-9_.-])
            ValueError: If two plugins have the same plugin_name
            ValueError: If some plugin handles a route that does not
                start with a slash
        """
        self._logdir = logdir
        ...
        self.data_applications = {
            DATA_PREFIX + LOGDIR_ROUTE:
                self._serve_logdir,
            # TODO(chizeng): Delete this RPC once we have skylark rules
            #                that obviate the need for the frontend to
        ...
        # Serve the routes from the registered plugins using their name
        # as the route prefix. For example if plugin z has two routes /a
        # and /b, they will be served as /data/plugin/z/a and
        # /data/plugin/z/b.
        ...
        for plugin in self._plugins:
            ...
            if not _VALID_PLUGIN_RE.match(plugin.plugin_name):
                raise ValueError('Plugin %s has invalid name %r' % (
            ...
            for route, app in plugin_apps.items():
                ...
                path = DATA_PREFIX + PLUGIN_PREFIX + '/' + plugin.plugin_name\
                       + route
    ...
    def _path_is_safe(self, path):
        ...
        Returns:
            True if the given path stays within the current directory,
            and false if it would escape to a higher directory. E.g.
            _path_is_safe('index.html') returns true, but
            _path_is_safe('../../../etc/password') returns false.
        """
    ...
    def _serve_plugins_listing(self, request):
        ...
        return http_util.Respond(
            request,
            {plugin.plugin_name: plugin.is_active() for plugin in
             self._plugins},
            'application/json')
    ...
    def _serve_runs(self, request):
        ...
        Returns a mapping from runs to tagType to list of tags for that
        run.
        ...
        def get_first_event_timestamp(run_name):
            try:
                ...
                tf.logging.warning(
                    'Unable to get first event timestamp for run %s',
                    run_name)
    ...
    def __call__(self, environ, start_response):  # pylint: disable=invalid-name
        ...
        This method handles routing to sub-applications. It does simple
        routing using regular expression matching.

        This __call__ method conforms to the WSGI spec, so that
        ...
        if clean_path in self.data_applications:
            ...
            return http_util.Respond(request, 'Not found', 'text/plain',
                                     code=404)(
...
def parse_event_files_spec(logdir):
    ...
    The events files flag format is a comma-separated list of path
    specifications. A path specification either looks like
    'group_name:/path/to/directory' or '/path/to/directory'; in the
    latter case, the group is unnamed. Group names cannot start with a
    forward slash: /foo:bar/baz will be interpreted as a spec with no
    name and path '/foo:bar/baz'.
    ...
    Returns:
        A dict mapping directory paths to names like
        {'/path/to/directory': 'name'}. Groups without an explicit name
        are named after their path.  If logdir is None, returns an empty
        dict, which is helpful for testing things that don't require any
        valid runs.
    """
    files = {}
    if logdir is None:
    ...
    for specification in logdir.split(','):
        # Check if the spec contains group. A spec start with xyz:// is
        # regarded as URI path spec instead of group spec. If the spec looks
        # like /foo:bar/baz, then we assume it's a path with a colon.
        if (uri_pattern.match(
            specification) is None and ':' in specification and
                    specification[0] != '/'):
...
def reload_multiplexer(multiplexer, path_to_run):
    ...
    Args:
        ...
        path_to_run: A dict mapping from paths to run names, where
            `None` as the run name is interpreted as a run name equal to
            the path.
    """
    start = time.time()
    ...
    tf.logging.info('TensorBoard done reloading. Load took %0.3f secs',
                    duration)
...
def start_reloading_multiplexer(multiplexer, path_to_run, load_interval):
    ...
    The thread will reload the multiplexer by calling
    `ReloadMultiplexer` every `load_interval` seconds, starting
    immediately.
    ...
    Args:
        ...
        path_to_run: A dict mapping from paths to run names, where
            `None` as the run name is interpreted as a run name equal to
            the path.
        load_interval: How many seconds to wait after one load before
...
def hacky_loader(path):
    """Load an asset from the tensorboard directory, in a platform-generic
    way."""
    try:
