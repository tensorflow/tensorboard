...
# Slashes in a plugin name could throw the router for a loop. An empty name
# would be confusing, too. To be safe, let's restrict the valid names as
# follows.
...
class TensorBoardWSGIApp(object):
    ...
    def __init__(self, logdir, plugins, multiplexer, reload_interval):
        ...
        Args:
            logdir: the logdir spec that describes where data will be loaded.
                ...
                or colons can be used to provide named directories
        ...
        self.data_applications = {
            DATA_PREFIX + LOGDIR_ROUTE:
                self._serve_logdir,
            # TODO(chizeng): Delete this RPC once we have skylark rules that
            # obviate
        ...
        # Serve the routes from the registered plugins using their name as
        # the route prefix. For example if plugin z has two routes /a and /b,
        # they will be served as /data/plugin/z/a and /data/plugin/z/b.
    ...
    def __call__(self, environ, start_response):  # pylint: disable=invalid-name
        ...
        This __call__ method conforms to the WSGI spec, so that instances of
        this class are WSGI applications.
...
def parse_event_files_spec(logdir):
    ...
    'group_name:/path/to/directory' or '/path/to/directory'; in the latter
    case, the group is unnamed. Group names cannot start with a forward
    slash: /foo:bar/baz will be interpreted as a spec with no name and path
    '/foo:bar/baz'.
    ...
    Returns:
        A dict mapping directory paths to names like {'/path/to/directory':
        'name'}. Groups without an explicit name are named after their path.
        If logdir is None, returns an empty dict, which is helpful for
        testing things that don't require any valid runs.
    """
    files = {}
    if logdir is None:
        return files
...
def reload_multiplexer(multiplexer, path_to_run):
    ...
    Args:
        ...
        path_to_run: A dict mapping from paths to run names, where `None` as
            the run name is interpreted as a run name equal to the path.
    """
    start = time.time()
