 ...
 */
export function categorizeBySearchQuery(xs: string[], query: string): RawCategory {
    const re = (() => {
        try {
...
export function categorizeTags(runToTag: RunToTag, selectedRuns: string[], query?: string): TagCategory[] {
...
function createRunToTagForPlugin(runs: tf_backend.Run[], pluginName: string): RunToTag {
    const runToTag = {};
    runs.forEach((run) => {
...
export function categorizeRunTagCombinations(runToTag: RunToTag, selectedRuns: string[], query?: string): RunTagCategory[] {
