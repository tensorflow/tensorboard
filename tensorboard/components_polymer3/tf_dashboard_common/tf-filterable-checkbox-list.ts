...
class TfFilterableCheckboxList extends PolymerElement {
    ...
    <div class="item">
      <paper-input id="input" autofocus="" class="input" no-label-float="" label="Write a regex to filter [[label]]s" value="[[_regexString]]" on-bind-value-changed="_debouncedRegexChange">
    </paper-input></div>
    <div class="matches">
      ...
      <template is="dom-repeat" items="[[_itemsMatchingRegex]]" on-dom-change="_synchronizeColors">
        <div class="item">
          <paper-checkbox checked$="[[_isChecked(item, selectionState.*)]]" class="checkbox" name="[[item]]" on-tap="_checkboxTapped" title="Alt+Click to select only [[item.title]].">
    ...
    _toggleAll() {
        let anyToggledOn = this._itemsMatchingRegex.some((n) => this.selectionState[n.id]);
