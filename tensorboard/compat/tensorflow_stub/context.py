...
class _ContextSwitchStack(threading.local):
    ...
    def push(self, is_building_function, enter_context_fn):
        ...
        self.stack.append(ContextSwitch(is_building_function, enter_context_fn))
...
class Context(object):
    ...
    def device(self, name):
        ...
        try:
            ...
            raise ValueError(
                "Expecting a string device name. Got %s(%s)" % (type(name), name)
            )
        except KeyError:
            # Handle a cache miss.
            if name is not None:
    ...
    def export_run_metadata(self):
        ...
        # run_metadata.ParseFromString(convert.as_bytes(proto_data))
        # return run_metadata
        pass
