...
export function makeBindings<T>(
  fromString: (dataString: string) => T,
  toString: (data: T) => string
): {
  get: (key: string, option?: StorageOptions<T>) => T | undefined;
  ...
  function get(key: string, options: StorageOptions<T> = {}): T | undefined {
 ...
 */
function unsetFromURI(key: string) {
  const items = componentToDict(readComponent());
  delete items[key];
