...
==============================================================================*/
export function htmlTableReporter(results) {
    const displayResults = results
        .map(({ totalTimeInMs, numIterations, timePerIterationInMs, timeInGcInMs, benchmark, }) => {
        if (numIterations <= 0) {
            throw new RangeError(`numIterations has to be positive value. Got ${numIterations}`);
        }
        const avgTimeInMs = totalTimeInMs / numIterations;
        const timeVariance = timePerIterationInMs
            .map((time) => {
        ...
        .map(({ name, min, max, numIterations, timeVariance, avgTimeInMs, avgTimeInGcInMs, }) => {
        return {
            name,
            numIterations,
    ...
    function createTHead() {
        ...
        headerRow.append(name, iterations, avgTime, min, max, stdDeviation, gc);
        header.appendChild(headerRow);
        return header;
    }
    // CREATE BODY
    const reporterContent = displayResults.map(({ avgTimeInMs, name, numIterations, minTimeInMs, maxTimeInMs, stdDeviationTimeInMs, avgTimeInGcInMs, }) => {
        ...
        row.append(nameEl, iterationsEl, avgTimeEl, minEl, maxEl, stdDeviationEl, gc);
        return row;
    });
    reporter.append(createTHead(), ...reporterContent);
    document.body.prepend(reporter);
}
export function consoleReporter(results) {
    const displayResults = results.map(({ benchmark, totalTimeInMs, numIterations }) => {
        return {
            name: benchmark.name,
            numIterations,
