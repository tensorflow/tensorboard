...
==============================================================================*/
var tf_wbr_string;
(function (tf_wbr_string) {
    const { expect } = chai;
    window.HTMLImports.whenReady(() => {
        describe('tf-wbr-string', () => {
            it('adds wbrs for patterns with a single character', (done) => {
                let testElement = fixture('tf-wbr-string');
                testElement.value = 'I/have/a/delimiter';
                testElement.delimiterPattern = '/';
                flush(() => {
                    expect(testElement.shadowRoot.innerHTML).to.have.string('I/<wbr>have/<wbr>a/<wbr>delimiter<wbr>');
                    expect(testElement.shadowRoot.textContent.trim()).to.equal('I/have/a/delimiter');
                    done();
                });
            });
            it('adds wbrs for patterns with multiple single characters', (done) => {
                ...
                flush(() => {
                    expect(testElement.shadowRoot.innerHTML).to.have.string('I_<wbr>have-<wbr>multiple_<wbr>delimiter.<wbr>s<wbr>');
                    done();
                });
            });
            it('adds wbrs for more complex patterns', (done) => {
                ...
                flush(() => {
                    expect(testElement.shadowRoot.innerHTML).to.have.string('the<wbr>a_heat<wbr>s_tea<wbr>');
                    done();
                });
            });
            it('ignores overlapped matches', (done) => {
                ...
                flush(() => {
                    ...
                    expect(testElement.shadowRoot.innerHTML).to.have.string('the<wbr>_the<wbr>atre_heat<wbr>s_tea<wbr>');
                    done();
                });
            });
            it('allows empty matches to consume remainder of the string', (done) => {
                ...
                testElement.delimiterPattern = '(the|eat)?';
                flush(() => {
                    expect(testElement.shadowRoot.innerHTML).to.have.string('the<wbr>_theatre_heats_tea<wbr>');
                    done();
                });
            });
            ...
            it('adds single wbr for empty pattern', (done) => {
                ...
                testElement.delimiterPattern = '';
                flush(() => {
                    expect(testElement.shadowRoot.innerHTML).to.have.string('Empty delimiter pattern<wbr>');
                    done();
                });
            });
            it('adds single wbr for undefined pattern', (done) => {
                ...
                flush(() => {
                    expect(testElement.shadowRoot.innerHTML).to.have.string('undefined pattern<wbr>');
                    done();
                });
            });
        });
    });
})(tf_wbr_string || (tf_wbr_string = {})); // namespace tf_wbr_string
