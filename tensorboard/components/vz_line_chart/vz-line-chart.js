...
(function (vz_line_chart) {
    ...
    Polymer({
        ...
        _computeTooltipTableHeaderHtml(tooltipColumns) {
            ...
            return titles
                .map((title) => `<th>${this._sanitize(title)}</th>`)
                .join('');
        },
        _sanitize(value) {
            return value
    ...
    class LineChart {
        ...
        buildChart(xComponentsCreationMethod, yValueAccessor, yScaleType, fillArea, xAxisFormatter, yAxisFormatter) {
            ...
            this.yAxis.formatter(yAxisFormatter
                ? yAxisFormatter
        ...
        resetYDomain() {
            ...
            if (this._defaultYRange != null) {
                ...
                const vals = _.flattenDeep(this.datasets.map(datasetToValues)).filter(isFinite);
            ...
            pi.onPointerMove((p) => {
                ...
                let ptsForTooltips = pts.filter((p) => (intersectsBBox(p.x, p.y, bbox) ||
                    ...
                    (!this.seriesWithoutTooltips ||
                        this.seriesWithoutTooltips.indexOf(p.dataset.metadata().name) ==
                            -1));
                ...
                let ptsSelection = this.tooltipPointsComponent
                    .content()
                    .selectAll('.point')
        ...
        drawTooltips(points, target, tooltipColumns) {
            ...
            if (this.tooltipSortingMethod === 'ascending') {
                ...
                points = _.sortBy(points, (d) => valueSortMethod(d.datum, -1, d.dataset)).reverse();
            }
