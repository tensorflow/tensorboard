  ...
  Polymer({
    ...
    exampleChanged: function() {
      ...
      this.changeCallbackTimer = setTimeout(
        ...
        CHANGE_CALLBACK_TIMER_DELAY_MS
      );
    },
    ...
    getInputPillClass: function(feat: string, displayMode: string) {
      return (
        this.sanitizeFeature(feat) +
        ' value-pill' +
        (displayMode == 'grid' ? ' value-pill-grid' : ' value-pill-stacked')
      );
    },

    getCompareInputClass: function(
      feat: string,
      displayMode: string,
      index?: number
    ) {
    ...
    getSeqCompareInputClass: function(
      feat: string,
      displayMode: string,
      seqNumber: number,
      index?: number
      ...
      if (index != null) {
        ...
        const compValues = this.getCompareSeqFeatureValues(
          feat,
          seqNumber,
          true
        );
    ...
    getImageSrc: function(feat: string) {
      ...
      return this.getImageSrcForData(feat, this.getFeatureValues(
        feat,
        false,
        true
      )[0] as string);
    ...
    getCompareImageSrc: function(feat: string) {
      ...
      return this.getImageSrcForData(
        feat,
        this.getCompareFeatureValues(feat, false, true)[0] as string,
        true
      );
    },
    ...
    getSeqImageSrc: function(feat: string, seqNum: number) {
      this.setupOnloadCallback(feat);
      return this.getImageSrcForData(feat, this.getSeqFeatureValues(
        feat,
        seqNum,
        false,
        true
    ...
    getCompareSeqImageSrc: function(feat: string, seqNum: number) {
      ...
      return this.getImageSrcForData(
        feat,
        this.getCompareSeqFeatureValues(feat, seqNum, false, true)[0] as string,
        true
      );
    },
    ...
    handleFileSelect: function(event: Event, self: any) {
      ...
      reader.addEventListener(
        ...
        () => {
          ...
          const index =
            +reader.result.indexOf(BASE_64_IMAGE_ENCODING_PREFIX) +
            ...
            img.addEventListener('load', () => {
              ...
              if (heights.length > 0) {
                ...
                self.features
                  .get(heightFeature)!
                  .getInt64List()!
                  .setValueList(heights as number[]);
              }
              self.exampleChanged();
            });
    ...
    addSaliencyToImage: function(
      d: Uint8ClampedArray,
      sal: SaliencyValue | SaliencyValue[]
    ) {
      ...
      const salScaleAdjustment =
        1 / Math.pow(this.imageScalePercentage / 100, 2);
    ...
    updateImages: function() {
      for (const feat in this.imageInfo) {
        if (this.imageInfo.hasOwnProperty(feat)) {
          this.imageInfo[feat].onload!(
            feat,
            this.imageInfo[feat].imageElement!
          );
        }
      }
    },
    ...
    getDeleteValueButtonClass: function(
      readonly: boolean,
      showDeleteValueButton: boolean
    ) {
    ...
    decodeBytesListToString: function(bytes: Uint8Array) {
      ...
      for (i = 0; i < bytes.length / decodeChunkBytes; i++) {
        res += String.fromCharCode.apply(
          null,
          bytes.slice(i * decodeChunkBytes, (i + 1) * decodeChunkBytes)
        );
      }
