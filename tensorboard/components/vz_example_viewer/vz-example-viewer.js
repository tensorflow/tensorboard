...
(function (vz_example_viewer) {
    ...
    Polymer({
        ...
        setFilteredFeaturesList: function (featureList, searchValue, saliency, sortOrder) {
            this.filteredFeaturesList = [];
            this.filteredFeaturesList = this.getFilteredFeaturesList(featureList, searchValue, saliency, sortOrder);
        },
        setFilteredSeqFeaturesList: function (seqFeatureList, searchValue, saliency, sortOrder) {
            this.filteredSeqFeaturesList = [];
            this.filteredSeqFeaturesList = this.getFilteredFeaturesList(seqFeatureList, searchValue, saliency, sortOrder);
        },
        getFilteredFeaturesList: function (featureList, searchValue, saliency, sortOrder) {
            if (featureList == null) {
                return;
            }
            let filtered = featureList;
            const checkSal = saliency &&
            ...
            const saliencyTotals = checkSal
                ? Object.assign({}, ...Object.keys(saliency).map((name) => ({
                    [name]: typeof saliency[name] == 'number'
                        ? saliency[name]
                        : saliency[name].reduce((total, cur) => total + cur, 0),
                })))
                : {};
            if (searchValue != '') {
        ...
        _haveSaliencyImpl: function () {
            ...
            for (const feat of this.filteredFeaturesList) {
                ...
                const colorFn = Array.isArray(val)
                    ? (d, i) => this.getColorForSaliency(val[i])
                    : () => this.getColorForSaliency(val);
                this.selectAll(`.${this.sanitizeFeature(feat.name)}.value-pill`)
                    .style('background', this.showSaliency ? colorFn : () => noAttributionColor)
                    .attr('title', (d, i) => 'Attribution: ' +
                ...
                if (Array.isArray(val)) {
                    const valArray = val;
                    const moreButton = this.selectAll(`paper-button.${this.sanitizeFeature(feat.name)}.value-pill`);
        ...
        seqSaliency: function () {
            ...
            for (const feat of this.seqFeaturesList) {
                const vals = this.saliency[feat.name];
                // If there is no saliency information for the feature, do not color it.
                if (!vals) {
                    continue;
                }
                ...
                const colorFn = Array.isArray(val)
                    ? (d, i) => this.getColorForSaliency(val[i])
                    : () => this.getColorForSaliency(val);
                this.selectAll(`.${this.sanitizeFeature(feat.name)} input`).style('color', this.showSaliency ? colorFn : () => 'black');
            }
        },
        /**
        ...
        getJsonValueList: function (feat, seqNum) {
            ...
            const valueList = feature.bytesList || feature.int64List || feature.floatList;
        ...
        getInputPillClass: function (feat, displayMode) {
            return (this.sanitizeFeature(feat) +
                ' value-pill' +
        ...
        setupOnloadCallback: function (feat, compare) {
            requestAnimationFrame(() => {
                const img = this.$$('#' + this.getImageId(feat, compare));
        ...
        parseFeatures: function (features) {
            ...
            for (const fname in features.feature) {
                if (features.feature.hasOwnProperty(fname)) {
                    ...
                    feats
                        .getFeatureMap()
                        .set(fname, this.parseFeature(featentry, this.isImage(fname)));
                }
            }
            return feats;
        },
            ...
            reader.addEventListener('load', () => {
                ...
                if (feat) {
                    ...
                    img.addEventListener('load', () => {
                        ...
                        const featureMiddle = IMG_FEATURE_REGEX.exec(data.feature)[1] || '';
        ...
        getOnLoadForImage: function (feat, image, compare) {
            const f = (feat, image, compare) => {
                const canvas = this.$$('#' + this.getCanvasId(feat, compare));
                if (!compare) {
                ...
                if (image && canvas) {
                    ...
                    if (!this.allowImageControls) {
                        const holder = this.$$('#' + this.getImageCardId(feat, compare))
                            .parentElement;
                    ...
                    const transformFn = (transform) => {
                        ...
                        this.renderImageOnCanvas(context, canvas.width, canvas.height, feat, compare);
                        context.restore();
                    };
                    const zoom = () => {
                    ...
                    d3.select(canvas)
                        .call(d3zoom)
                        .on('dblclick.zoom', () => d3.select(canvas).call(d3zoom.transform, d3.zoomIdentity));
                    context.save();
        ...
        contrastImage: function (d, windowWidth, windowCenter) {
            ...
            const contrastScale = d3
                .scaleLinear()
                .domain([
                windowCenter - 0.5 - windowWidth / 2,
                windowCenter - 0.5 + (windowWidth - 1) / 2,
            ])
                .clamp(true)
                .range([0, 255]);
            ...
            for (let i = 0; i < d.length; i += 4) {
                ...
                const ratioToSaliencyExtreme = this.showSaliencyForValue(salVal)
                    ? salVal >= 0
                        ...
                        : salVal / this.minSal
                    : 0;
                const blendRatio = IMG_SALIENCY_MAX_COLOR_RATIO * ratioToSaliencyExtreme;
                const { r, g, b } = d3.rgb(salVal > 0 ? this.colors(this.maxSal) : this.colors(this.minSal));
        ...
        getDeleteValueButtonClass: function (readonly, showDeleteValueButton) {
            return readonly || !showDeleteValueButton
