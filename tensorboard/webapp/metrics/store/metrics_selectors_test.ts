...
describe('metrics selectors', () => {
  ...
  describe('getMetricsImageCardStepValues', () => {
    beforeEach(() => {
      selectors.getMetricsImageCardStepValues.release();
    });

    it('returns multiple step value', () => {
      const state = appStateFromMetricsState(
        buildMetricsState({
          timeSeriesData: {
            ...createTimeSeriesData(),
      ...
      expect(selectors.getMetricsImageCardStepValues(state, 'card1')).toEqual([
        0, 1, 99,
      ]);
    });

    it('returns single step value', () => {
      const state = appStateFromMetricsState(
        buildMetricsState({
          timeSeriesData: {
            ...createTimeSeriesData(),
            images: {
              tagB: {
                1111: {
                  runToLoadState: {run1: DataLoadState.LOADED},
                  runToSeries: {
                    run1: [{step: 101, wallTime: 125, imageId: '<image_id_2>'}],
                  },
                },
              },
            },
    ...
    it('returns empty step value if no card id exists in cardMetadataMap', () => {
      const state = appStateFromMetricsState(buildMetricsState({}));
      expect(selectors.getMetricsImageCardStepValues(state, 'card1')).toEqual(
        []
      );
    ...
    it('returns empty step value when no steps in the run', () => {
      const state = appStateFromMetricsState(
        buildMetricsState({
          timeSeriesData: {
            ...createTimeSeriesData(),
    ...
    it('returns empty step value when no run id matched time series data', () => {
      const state = appStateFromMetricsState(
        buildMetricsState({
          timeSeriesData: {
            ...createTimeSeriesData(),
            images: {
              tagB: {
                1111: {
                  runToLoadState: {run1: DataLoadState.LOADED},
                  runToSeries: {
                    run2: [{step: 101, wallTime: 125, imageId: '<image_id_2>'}],
                  },
                },
              },
            },
    ...
    it('returns empty step value when no tag matched time series data', () => {
      const state = appStateFromMetricsState(
        buildMetricsState({
          cardMetadataMap: {
            card1: {
