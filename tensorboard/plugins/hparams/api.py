...
To control how hyperparameters and metrics appear in the TensorBoard
frontend, you can define `HParam` and `Metric` objects and collect them
in an `Experiment`:
...
...   hp.experiment(experiment)  # write experiment summary

You can continue to pass a string-keyed dict to the Keras callback, or
you can use `HParam` objects as the keys. The latter approach enables
better static analysis: your favorite Python linter can tell you if you
misspell a hyperparameter name, your IDE can help you find all the
places where a hyperparameter is used, etc:
