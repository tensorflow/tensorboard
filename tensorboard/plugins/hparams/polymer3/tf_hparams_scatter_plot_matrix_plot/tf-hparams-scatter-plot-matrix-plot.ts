...
class TfHparamsScatterPlotMatrixPlot extends PolymerElement {
    ...
    _sessionGroupsChanged() {
        if (this.selectedSessionGroup !== null) {
            ...
            this._setSelectedSessionGroup(tf.hparams.utils.sessionGroupWithName(this.sessionGroups, this.selectedSessionGroup.name) || null);
        }
        this._redraw();
    }
    ...
    _draw() {
        ...
        xAxesG
            ...
            .each(function (col) {
            d3.select(this).call(drawAxis, d3
                .axisBottom(xCoords[col])
        ...
        if (isBrushActive()) {
            brushedCellG = cells.filter((cellIndex) => _.isEqual(cellIndex, _this._brushedCellIndex));
        ...
        function updateVisibleMarkers() {
            ...
            d3.selectAll(Array.from(utils.filterSet(newVisibleMarkers, (elem) => !visibleMarkers.has(elem)))).attr('fill', markerColorFn);
            // Gray-out the no-longer visible markers.
            d3.selectAll(Array.from(utils.filterSet(visibleMarkers, (elem) => !newVisibleMarkers.has(elem)))).attr('fill', '#ddd');
        ...
        function brushChanged(cellGNode) {
            ...
            if ((!isBrushActive() && brushSelection === null) ||
                (isBrushActive() &&
    ...
    _cellScale(colIndex, range) {
        ...
        if (this.options.columns[colIndex].scale === 'LINEAR') {
            ...
            return d3
                .scaleQuantile()
                .domain(_.uniq(this.sessionGroups.map((sg) => this._colValue(sg, colIndex))))
                .range(scaleRange);
        }
            ...
            _.uniq(this.sessionGroups
                .map((sg) => this._colValue(sg, colIndex))
                .sort()))
                .range(range)
                .padding(0.1);
        }
    ...
    _colValue(sessionGroup, colIndex) {
        return tf.hparams.utils.columnValueByVisibleIndex(this.visibleSchema, sessionGroup, colIndex);
    }
    _metricValue(sessionGroup, metricIndex) {
        return tf.hparams.utils.metricValueByVisibleIndex(this.visibleSchema, sessionGroup, metricIndex);
    }
    _colExtent(colIndex) {
        return tf.hparams.utils.visibleNumericColumnExtent(this.visibleSchema, this.sessionGroups, colIndex);
    }
}
