...
class TfHparamsSessionGroupDetails extends PolymerElement {
    static readonly template = html `<template is="dom-if" if="[[!sessionGroup]]">
      <div>
    ...
    _sessionGroupChanged() {
        if (!this.sessionGroup) {
            ...
            this._sessionGroupNameHash = tf.hparams.utils.hashOfString(this.sessionGroup.name);
        }
    ...
    _computeSeriesForSessionGroupMetric(sessionGroup, metricInfo) {
        ...
        (session) => tf.hparams.utils.metricValueByName(session.metricValues, metricInfo.name) !== undefined)
            .map(
