...
namespace memory_viewer_usage {
  ...
  export class MemoryUsage {
    ...
    private newHeapObject_(
      color: number,
      buffer: memory_viewer_xla_lb.LogicalBuffer,
    ...
    private findPeakMemoryUsage_(heapSimulatorTrace) {
      ...
      for (const event of heapSimulatorTrace.events) {
        ...
        if (buffer.instructionName && buffer.instructionName != '') {
          shape = this.nameToHlo_[
            buffer.instructionName
          ].shape.resolveShapeIndex(buffer.shapeIndex);
        }
      ...
      heapSizes.push(memory_viewer_utils.bytesToMiB(heapSizeBytes));
      const indefiniteMemoryUsageBytes = this.findIndefiniteMemoryUsage_(
        this.unSeenLogicalBuffers_
      );
