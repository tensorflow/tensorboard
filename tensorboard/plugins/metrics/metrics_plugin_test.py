...
class MetricsPluginTest(tf.test.TestCase):
  ...
  def setUp(self):
    ...
    provider = data_provider.MultiplexerDataProvider(self._multiplexer,
                                                     self._logdir)
  ...
  def test_tags_unsafe_description(self):
    ...
    self._write_histogram("<&#run>", "histograms/<&#tag>", "<&#description>")
    self._multiplexer.Reload()
                ...
                "run1": [
                    {
                        "wallTime": "<wall_time>",
                        "step": 0,
                        "bins": [{"min": -0.5, "max": 0.5, "count": 1.0}],
                    },
                    {
                        "wallTime": "<wall_time>",
                        "step": 1,
                        "bins": [{"min": 9.5, "max": 10.5, "count": 1.0}],
                    },
                ]
            },
        }],
  ...
  def test_image_data(self):
    ...
    image_id = self._get_image_blob_key("run1", "images/tagA", step=0, sample=0)
  ...
  def test_time_series_bad_arguments(self):
    ...
    errors = [series_response.get("error", "") for series_response in response]
