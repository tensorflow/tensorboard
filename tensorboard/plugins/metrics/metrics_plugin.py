...
class MetricsPlugin(base_plugin.TBPlugin):
  ...
  def __init__(self, context):
    ...
    self._plugin_downsampling = {
        "scalars":
            sampling_hints.get(scalar_metadata.PLUGIN_NAME, 1000),
        "histograms":
            sampling_hints.get(histogram_metadata.PLUGIN_NAME, 51),
        "images":
            sampling_hints.get(image_metadata.PLUGIN_NAME, 10),
    }
  ...
  def _get_invalid_request_error(self, series_request):
    ...
    if (plugin != scalar_metadata.PLUGIN_NAME and
        plugin != histogram_metadata.PLUGIN_NAME and
        plugin != image_metadata.PLUGIN_NAME):
