...
def run(logdir, run_name,
    ...
    This will simulate an object at temperature `initial_temperature` sitting
    at rest in a large room at temperature `ambient_temperature`. The object
    has some intrinsic `heat_coefficient`, which indicates how much thermal
    conductivity it has: for instance, metals have high thermal conductivity,
    while the thermal conductivity of water is low.

    Over time, the object's temperature will adjust to match the temperature
    of its environment. We'll track the object's temperature, how far it is
    from the room's temperature, and how much it changes at each time step.
    ...
    with tf.name_scope('temperature'):
        ...
        # and create a scalar summary to track its value over time. The name
        # of the summary will appear as "temperature/current" due to the
        # name-scope above.
        ...
        # environment, and track this, too: likewise,
        # as "temperature/difference_to_ambient".
    ...
    # coefficient. But in real life, not everything is quite so clean,
    # so we'll add in some noise. (The value of 50 is arbitrary, chosen to
