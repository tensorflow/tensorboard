...
def run(logdir, run_name,
    ...
    sitting at rest in a large room at temperature
    `ambient_temperature`.  The object has some intrinsic
    `heat_coefficient`, which indicates how much thermal conductivity it
    has: for instance, metals have high thermal conductivity, while the
    thermal conductivity of water is low.

    Over time, the object's temperature will adjust to match the
    temperature of its environment. We'll track the object's
    temperature, how far it is from the room's temperature, and how much
    it changes at each time step.

    Arguments:
        logdir: The top-level directory into which to write summary
            data.
        run_name: The name of this run; will be created as a
            subdirectory under logdir.
        initial_temperature: float; the object's initial temperature
        ambient_temperature: float; the temperature of the enclosing
            room.
    ...
    with tf.name_scope('temperature'):
        # Create a mutable variable to hold the object's temperature,
        # and create a scalar summary to track its value over time. The
        # name of the summary will appear as "temperature/current" due
        # to the name-scope above.
    ...
    # object is directly proportional to this `ambient_difference`
    # above, where the proportionality constant is what we called the
    # heat coefficient. But in real life, not everything is quite so
    # clean, so we'll add in some noise. (The value of 50 is arbitrary,
    # chosen to make the data look somewhat interesting. :-) )
    noise = 50 * tf.random_normal([])
    delta = -heat_coefficient * (ambient_difference + noise)
    tf.summary.scalar('delta', delta)

    # Now, augment the current temperature by this delta that we
    # computed.
    ...
    for step in xrange(STEPS):
        # By asking TensorFlow to compute the update step, we force it
        # to change the value of the temperature variable. We don't
        # actually care about this value, so we discard it; instead, we
        # grab the summary data computed along the way.
