...
(function (tf_histogram_dashboard) {
    describe('Verify that the histogram format conversion works.', function () {
        ...
        it('Handles data with no bins', function () {
            chai.assert.deepEqual(tf_histogram_dashboard.intermediateToD3({
                wall_time: 1234.5,
                step: 0,
                min: 0,
                max: 0,
        ...
        it('Handles data with one bin', function () {
            var buckets = [{ left: 1.1e-12, right: 1.21e-12, count: 1 }];
            var expected = [{ x: 1.1e-12, dx: 1.21e-12 - 1.1e-12, y: 1 }];
            var actual = tf_histogram_dashboard.intermediateToD3({
                wall_time: 1234.5,
                step: 0,
                min: 1.1e-12,
                max: 1.21e-12,
        ...
        it('Handles a histogram of all zeros', function () {
            ...
            var actual = tf_histogram_dashboard.intermediateToD3(h, 0, 0, 5);
            var expected = [
                { x: -1, dx: 0.4, y: 0 },
                { x: -0.6, dx: 0.4, y: 0 },
