...
class VzHistogramTimeseries extends PolymerElement {
    static readonly template = html `<div id="tooltip"><span></span></div>
    <svg id="svg">
      <g>
        <g class="axis x"></g>
        <g class="axis y"></g>
    ...
    _draw(duration) {
        ...
        var leftMin = d3.min(data, xAccessor), rightMax = d3.max(data, xRightAccessor);
        //
        // Text formatters
        //
        ...
        var histogram = stage.selectAll('.histogram').data(data), histogramExit = histogram.exit().remove(), histogramEnter = histogram
            .enter()
            .append('g')
            .attr('class', 'histogram'), histogramUpdate = histogramEnter
            .merge(histogram)
            .sort(function (a, b) {
            ...
            return ('translate(0, ' +
                (mode === 'offset'
                    ? yScale(timeAccessor(d)) - sliceHeight
                    : 0) +
                ')');
        });
        var baselineEnter = histogramEnter
            .append('line')
            .attr('class', 'baseline'), baselineUpdate = histogramTransition
            .select('.baseline')
            ...
            .attr('y2', sliceHeight)
            .attr('x2', width);
        var outlineEnter = histogramEnter
            .append('path')
            .attr('class', 'outline'), outlineUpdate = histogramUpdate
            .select('.outline')
        ...
        var xAxisHover = g
            .select('.x-axis-hover')
            .selectAll('.label')
            .data(['x']), xAxisHoverEnter = xAxisHover
            .enter()
            .append('g')
            .attr('class', 'label'), xAxisHoverUpdate = xAxisHover.merge(xAxisHoverEnter);
        xAxisHoverEnter
            .append('rect')
            .attr('x', -20)
            .attr('y', 6)
        ...
        var yAxisHover = g
            .select('.y-axis-hover')
            .selectAll('.label')
            .data(['y']), yAxisHoverEnter = yAxisHover
            .enter()
            .append('g')
            .attr('class', 'label'), yAxisHoverUpdate = yAxisHover.merge(yAxisHoverEnter);
        yAxisHoverEnter
            .append('rect')
            .attr('x', 8)
            .attr('y', -6)
        ...
        function onMouseMove() {
            var m = d3.mouse(this), v = xScale.invert(m[0]), t = yScale.invert(m[1]);
            ...
            hoverUpdate.attr('transform', function (d, i) {
                ...
                var globalY = mode === 'offset'
                    ? yScale(timeAccessor(d)) - (sliceHeight - y)
                    : y;
