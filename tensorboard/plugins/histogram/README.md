...
If you have pre-computed the heights of `N` bins, you can use the `edges` parameter to specify the edges of the bins and the `data` parameter to pass their heights. 

For instance, if you wish to visualize a softmax vector of 5 classes: `[0.1, 0.3, 0.2, 0.0, 0.4]` you could simply pass the softmax vector as `data` and specify `edges` as `[0.5, 1.5, 2.5, 3.5, 4.5, 5.5]`:
```
