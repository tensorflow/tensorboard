...
class FreezeSuggestion(Suggestion):
    ...
    def _get_error_map(cls):
        return {"Input checkpoint": "please check your checkpoint argument"}
        ...
        if input_saver_def:
            ...
            try:
                ...
                if has_partition_var:
                    raise ValueError("Models containing partition variables cannot be converted "
                          "from checkpoint files. Please pass in a SavedModel using "
                          "the flag --input_saved_model_dir.")
                # Models that have been frozen previously do not contain Variables.
                elif _has_no_variables(sess):
                    raise ValueError("No variables were found in this model. It is likely the model "
...
def freeze_with_suggestion(args):
    try:
        run_main(args)
    except Exception as e:
        suggestion = FreezeSuggestion(e)
        raise suggestion
