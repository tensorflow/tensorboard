...
class TFLiteConverterSuggestion(suggestion.Suggestion):
    def __init__(self, from_exception, stack_trace=None):
        suggestion.Suggestion.__init__(self, from_exception, stack_trace)

    @classmethod
    def _get_error_map(cls):
        return {"both shapes must be equal": "please input your input_shapes"}
...
def run_toco(args):
    ...
    tflite_flags, unparsed = parser.parse_known_args(args=args)
    _check_flags(tflite_flags, unparsed)
    _convert_model(tflite_flags)


def run_toco_with_suggestion(args):
    try:
        run_toco(args)
    except Exception as e:
        raise TFLiteConverterSuggestion(e, traceback.format_exc())
