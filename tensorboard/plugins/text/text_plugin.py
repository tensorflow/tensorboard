...
WARNING_TEMPLATE = textwrap.dedent("""\
    **Warning:** This text summary contained data of dimensionality %d,
    but only \
...
def markdown_and_sanitize(markdown_string):
    ...
    The sanitizer uses the allowed_tags and attributes specified
    above. Mostly, we ensure that our standard use cases like tables and
    links are supported.
...
def make_table_row(contents, tag='td'):
    ...
    Args:
        ...
        tag: The tag to place contents in. Defaults to 'td', you might
            want 'th'.

    Returns:
        A string containing the content strings, organized into a table
        row.
...
def make_table(contents, headers=None):
    ...
    Args:
        contents: A np.ndarray of strings. May be 1d or 2d. In the 1d
            case, the table is laid out vertically (i.e. row-major).
        headers: A np.ndarray or list of string header names for the
            table.

    Returns:
        A string containing all of the content strings, organized into a
        table.
    ...
    Raises:
        ...
        ValueError: If headers is present and not a list, tuple, or
            ndarray.
        ValueError: If headers is not 1d.
        ValueError: If number of elements in headers does not correspond
            to number of columns in contents.
    """
    ...
    if contents.ndim == 1:
        # If it's a vector, we need to wrap each element in a new list,
        # otherwise we would turn the string itself into a row (see test
        # code)
...
def reduce_to_2d(arr):
    ...
    It does this by selecting the zeroth coordinate for every dimension
    greater than two.
    ...
    Raises:
        ValueError: If the argument is not a numpy ndarray, or the
...
def text_array_to_html(text_arr):
    ...
    If the ndarray contains a single scalar string, that string is
    converted to html via our sanitized markdown parser. If it contains
    an array of strings, the strings are individually converted to html
    and then composed into a table using make_table. If the array
    contains dimensionality greater than 2, all but two of the
    dimensions are removed, and a warning message is prefixed to the
    table.
...
class TextPlugin(base_plugin.TBPlugin):
    ...
    def is_active(self):
        ...
        This plugin is only active if TensorBoard sampled any text
        summaries.
