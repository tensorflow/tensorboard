...
def make_table_row(contents, tag='td'):
    ...
    Args:
        ...
        tag: The tag to place contents in. Defaults to 'td', you might want
            'th'.
...
def make_table(contents, headers=None):
    ...
    Raises:
        ...
        ValueError: If number of elements in headers does not correspond to
            number of columns in contents.
    """
    ...
    if contents.ndim == 1:
        ...
        # otherwise we would turn the string itself into a row (see test code)
...
def text_array_to_html(text_arr):
    ...
    If the ndarray contains a single scalar string, that string is converted
    to html via our sanitized markdown parser. If it contains an array of
    strings, the strings are individually converted to html and then composed
    into a table using make_table. If the array contains dimensionality
    greater than 2, all but two of the dimensions are removed, and a warning
    message is prefixed to the table.
