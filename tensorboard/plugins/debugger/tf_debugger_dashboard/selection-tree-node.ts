...
namespace tf_debugger_dashboard {
  ...
  export const DEVICE_NAME_PATTERN = /^\/job:[A-Za-z0-9_]+\/replica:[0-9_]+\/task:[0-9]+\/device:[A-Za-z0-9_]+:[0-9]+/;
  ...
  export function splitNodeName(name: string): string[] {
    ...
    if (deviceNameMatches != null) {
      ...
      if (nodeName[deviceNameMatches[0].length] !== '/') {
        console.error(
          'No slash ("/") after device name in node name:',
          nodeName
        );
      }
  ...
  export function getCleanNodeName(name: string): string {
    ...
    if (deviceNameMatches != null) {
      if (
        cleanName.length > deviceNameMatches[0].length &&
  ...
  export function filterDebugWatches(
    ...
    if (filterMode === DebugWatchFilterMode.NodeName) {
      return debugWatches.filter((debugWatch) =>
        debugWatch.node_name.match(filterRegex)
      );
    } else if (filterMode === DebugWatchFilterMode.OpType) {
      return debugWatches.filter((debugWatch) =>
        debugWatch.op_type.match(filterRegex)
      );
    }
  }
      ...
      while (currentNode) {
        if (partialFound) {
          ...
          currentNode.setCheckboxState(
            partialFound ? CheckboxState.PARTIAL : CheckboxState.CHECKED,
            true
          );
        }
        // Move up the tree.
    ...
    setNodesAboveToEmpty() {
      ...
      while (currentNode) {
        if (partialFound) {
          ...
          partialFound = index !== -1;
          currentNode.setCheckboxState(
            partialFound ? CheckboxState.PARTIAL : CheckboxState.EMPTY,
            true
          );
        }
        // Move up the tree.
