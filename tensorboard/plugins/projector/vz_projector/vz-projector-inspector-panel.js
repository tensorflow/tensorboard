...
(function (vz_projector) {
    ...
    class InspectorPanel extends vz_projector.InspectorPanelPolymer {
        ready() {
            ...
            this.setFilterButton = this.$$('.set-filter');
            this.clearSelectionButton = this.$$('.clear-selection');
        ...
        metadataEditorContext(enabled, metadataColumn) {
            ...
            entries.forEach((e) => {
                ...
                labelElement.style.color = vz_projector.dist2color(this.distFunc, maxCount, e.count);
                ...
                barFillElement.style.borderTopColor = vz_projector.dist2color(this.distFunc, maxCount, e.count);
        ...
        updateNeighborsList(neighbors) {
            ...
            for (let i = 0; i < neighbors.length; i++) {
                ...
                const labelElement = document.createElement('div');
                labelElement.className = 'label';
                labelElement.style.color = vz_projector.dist2color(this.distFunc, neighbor.dist, minDist);
                ...
                const barFillElement = document.createElement('div');
                barFillElement.className = 'fill';
                barFillElement.style.borderTopColor = vz_projector.dist2color(this.distFunc, neighbor.dist, minDist);
                barFillElement.style.width =
