...
#
# TODO(dandelionmane): Fix duplication when we find a permanent home for
...
class EmbeddingMetadata(object):
    ...
    def add_column(self, column_name, column_values):
        ...
        Args:
            ...
            column_values: 1D array/list/iterable holding the column
                values.  Must be of length `num_points`. The i-th value
                corresponds to the i-th point.

        Raises:
            ValueError: If `column_values` is not 1D array, or of length
                `num_points`, or the `name` is already used.
        """
        # Sanity checks.
        if isinstance(column_values, list) and isinstance(column_values[0],
                                                          list):
            raise ValueError(
...
class ProjectorPlugin(base_plugin.TBPlugin):
    ...
    def _read_latest_config_files(self, run_path_pairs):
        ...
        for run_name, assets_dir in run_path_pairs:
            ...
            if (config.model_checkpoint_path and
                    not tf.train.checkpoint_exists(
    ...
    def _append_plugin_asset_directories(self, run_path_pairs):
        for run, assets in self.multiplexer.PluginAssets(_PLUGIN_NAME).items():
            if PROJECTOR_FILENAME not in assets:
                continue
            assets_dir = os.path.join(self.run_paths[run], _PLUGINS_DIR,
                                      _PLUGIN_NAME)
    ...
    def _serve_metadata(self, request):
        ...
        if not fpath:
            return Respond(
                request,
                'No metadata file found for tensor "%s" in the config file '
                '"%s"' %
        ...
        with tf.gfile.GFile(fpath, 'r') as f:
            lines = []
            # Stream reading the file with early break in case the file
            # doesn't fit in
            # memory.
            for line in f:
                lines.append(line)
        ...
        if tensor is None:
            ...
            if embedding and embedding.tensor_path:
                ...
                if not reader or not reader.has_tensor(name):
                    return Respond(request,
                                   'Tensor "%s" not found in checkpoint dir '
                                   '"%s"' %
    ...
    def _serve_bookmarks(self, request):
        ...
        if not fpath:
            return Respond(
                request,
                'No bookmarks file found for tensor "%s" in the config file '
                '"%s"' %
                (name, self.config_fpaths[run]), 'text/plain', 400)
        fpath = _rel_to_abs_asset_path(fpath, self.config_fpaths[run])
        if not tf.gfile.Exists(fpath) or tf.gfile.IsDirectory(fpath):
    ...
    def _serve_sprite_image(self, request):
        ...
        if not embedding_info or not embedding_info.sprite.image_path:
            return Respond(
                request,
                'No sprite image file found for tensor "%s" in the config '
                'file "%s"' %
                (name, self.config_fpaths[run]), 'text/plain', 400)
