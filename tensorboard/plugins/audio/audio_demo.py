...
def run(logdir, run_name, wave_name, wave_constructor):
    ...
    The provided function `wave_constructor` should accept a scalar
    tensor of type float32, representing the frequency (in Hz) at which
    to construct a wave, and return a tensor of shape [1, _samples(),
    `n`] representing audio data (for some number of channels `n`).
    ...
    # On each step `i`, we'll set this placeholder to `i`. This allows
    # us to know "what time it is" at each step.
    step_placeholder = tf.placeholder(tf.float32, shape=[])

    # We want to linearly interpolate a frequency between A4 (440 Hz)
    # and A5 (880 Hz).
    f_min = 440.0
    f_max = 880.0
    ...
    # Let's log this frequency, just so that we can make sure that it's
    # as expected.
    tf.summary.scalar('frequency', frequency)

    # Now, we pass this to the wave constructor to get our
    # waveform. Doing so within a name scope means that any summaries
    # that the wave constructor produces will be namespaced.
...
def triangle_wave(frequency):
    ...
    # diagram above), we can see that it looks like a transformed
...
def bisine_wave(frequency):
    ...
    #
    # Now, we have two tensors of shape [1, _samples(), 1]. By
    # concatenating them along axis 2, we get a tensor of shape [1,
    # _samples(), 2]---a stereo waveform.
...
def bisine_wahwah_wave(frequency):
    ...
    #
    # This is clearly intended to build on the bisine wave defined
    # above, so we can start by generating that.
