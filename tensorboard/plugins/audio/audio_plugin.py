...
class AudioPlugin(base_plugin.TBPlugin):
    ...
    def _serve_audio_metadata(self, request):
        ...
        Note that the audio themselves are not sent; instead, we respond
    ...
    def _audio_response_for_run(self, run_audio, run, tag):
        ...
        Args:
            run_audio: A list of event_accumulator.AudioValueEvent
                objects.
    ...
    def _query_for_individual_audio(self, run, tag, index):
        ...
        This should be kept in sync with _serve_audio_metadata. Note
        that the URL is *not* guaranteed to always return the same
        audio, since audio may be unloaded from the reservoir as new
        audio entries come in.
