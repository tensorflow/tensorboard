...
class AudioPlugin(base_plugin.TBPlugin):
    ...
    def _serve_audio_metadata(self, request):
        ...
        Note that the audio themselves are not sent; instead, we respond with
        URLs to the audio. The frontend should treat these URLs as opaque and
        should not try to parse information about them or generate them
        itself, as the format may change.
    ...
    def _audio_response_for_run(self, run_audio, run, tag):
        ...
        Returns:
            ...
            width, and height for each audio entry.
        """
        response = []
    ...
    def _query_for_individual_audio(self, run, tag, index):
        ...
        This should be kept in sync with _serve_audio_metadata. Note that the
        URL is *not* guaranteed to always return the same audio, since audio
        may be unloaded from the reservoir as new audio entries come in.
