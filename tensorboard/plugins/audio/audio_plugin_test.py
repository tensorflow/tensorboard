...
class AudioPluginTest(tf.test.TestCase):
    def setUp(self):
        ...
        # We use numpy.random to generate audio. We seed to avoid
    ...
    def testIndividualAudioRoute(self):
        ...
        response = self.server.get(
            "/data/plugin/audio/individualAudio?run=bar&tag=quux/audio/0"
            "&index=0")
        self.assertEqual(200, response.status_code)
    ...
    def testRunsRoute(self):
        ...
        self.assertItemsEqual(
            ["quux/audio/0", "quux/audio/1", "quux/audio/2"],
            run_to_tags["bar"])
