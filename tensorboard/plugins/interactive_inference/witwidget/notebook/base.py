...
class WitWidgetBase(object):
    ...
    def __init__(self, config_builder):
        ...
        if self.config.get("uses_json_input"):
            if self.custom_predict_fn is not None and not self.config.get(
                "use_aip"
            ):
            ...
            if (
                self.compare_custom_predict_fn is not None
                and not self.config.get("compare_use_aip")
            ):
                ...
                self.compare_custom_predict_fn = (
                    wrapped_compare_custom_predict_fn
                )
    ...
    def set_examples(self, examples):
        ...
        The examples are initially set by the examples specified in the
        config builder during construction. This method can change which
        examples WIT displays.
        """
        if self.config.get("uses_json_input"):
            tf_examples = self._json_to_tf_examples(examples)
            self.examples = [
                json_format.MessageToJson(ex) for ex in tf_examples
            ]
        else:
    ...
    def infer_impl(self):
        ...
        if (
            "inference_address_2" in self.config
    ...
    def sort_eligible_features_impl(self, info):
        ...
        for feat in features_list:
            chart_data[feat["name"]] = self.infer_mutants_impl(
                {
                    "x_min": feat["observedMin"]
                    if "observedMin" in feat
                    else 0,
                    "x_max": feat["observedMax"]
                    if "observedMin" in feat
                    else 0,
        ...
        return inference_utils.sort_eligible_features(features_list, chart_data)
    ...
    def create_sprite(self):
        ...
        feature_list = (
            example_to_check.context.feature
            if self.config.get("are_sequence_examples")
            ...
            if self.config.get("uses_json_list"):
                ...
                for feat in ex.features.feature:
                    ...
                    if ex.features.feature[feat].HasField("int64_list"):
                        json_ex[feat_idx] = ex.features.feature[
                            feat
                        ].int64_list.value[0]
                ...
                for feat in ex.features.feature:
                    if feat == self.config.get("target_feature"):
                        continue
                    if ex.features.feature[feat].HasField("int64_list"):
                        json_ex[feat] = ex.features.feature[
                            feat
                        ].int64_list.value[0]
                    elif ex.features.feature[feat].HasField("float_list"):
                        json_ex[feat] = ex.features.feature[
    ...
    def _predict_aip_impl(
        ...
        """Custom prediction function for running inference through AI
        Platform."""
        ...
        def predict(exs):
            ...
            request_builder = service.projects().predict(
                name=name, body={"instances": examples_for_predict}
            )
            ...
            if should_explain and not error_during_prediction:
                try:
                    ...
                    explanations = [
                        explain["attributions_by_label"][0]["attributions"]
        ...
        for response in responses:
            if "explanations" in response:
                ...
                if adjust_attribution is not None:
                    all_attributions.extend(
                        [
                            adjust_attribution(attr)
                            for attr in response["explanations"]
                        ]
                    )
                else:
            ...
            for pred in response["predictions"]:
                ...
                if self.config.get("model_type") == "regression" and isinstance(
                    pred, list
                ):
                    pred = pred[0]
            ...
            for ex in examples:
                if selected["sliceName"] == "Overall":
                    ...
                    values = extract_values(
                        ex.features.feature[selected["sliceName"]]
                    )
