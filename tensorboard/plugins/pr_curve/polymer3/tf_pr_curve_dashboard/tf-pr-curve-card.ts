...
class TfPrCurveCard extends PolymerElement {
    ...
    _dataUrl: object = function () {
        return (run) => {
            const tag = this.tag;
            return tf_backend.addParams(tf_backend.getRouter().pluginRoute('pr_curves', '/pr_curves'), { tag, run });
        };
    };
    @property({
        type: Boolean,
    ...
    _updateSeriesDataForRun(run, entryForOneStep) {
        ...
        const fieldsToData = _.reduce(this._seriesDataFields, (result, field) => {
    ...
    _computeEntryClosestOrEqualToStepCap(stepCap, entries) {
        const entryIndex = Math.min(_.sortedIndex(entries.map((entry) => entry.step), stepCap), entries.length - 1);
