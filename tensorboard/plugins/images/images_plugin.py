...
class ImagesPlugin(base_plugin.TBPlugin):
    ...
    def _serve_image_metadata(self, request):
        ...
        Note that the images themselves are not sent; instead, we respond
        with URLs to the images. The frontend should treat these URLs as
        opaque and should not try to parse information about them or generate
        them itself, as the format may change.
    ...
    def _image_response_for_run(self, run_images, run, tag):
        ...
        for index, run_image in enumerate(run_images):
            response.append({
                ...
                'step': run_image.step,
                # We include the size so that the frontend can add that to
                # the <img> tag so that the page layout doesn't change when
                # the image loads.
    ...
    def _query_for_individual_image(self, run, tag, index):
        ...
        This should be kept in sync with _serve_image_metadata. Note that the
        URL is *not* guaranteed to always return the same image, since images
        may be unloaded from the reservoir as new images come in.
