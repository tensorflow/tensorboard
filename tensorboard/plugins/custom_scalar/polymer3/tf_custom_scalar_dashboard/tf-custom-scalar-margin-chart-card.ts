...
class TfCustomScalarMarginChartCard extends PolymerElement {
    static readonly template = html `<tf-card-heading display-name="[[_titleDisplayString]]"></tf-card-heading>
    ...
    _dataUrl: object = function () {
        return (run) => {
            ...
            return tf_backend.addParams(tf_backend
                .getRouter()
                .pluginRoute('custom_scalars', '/scalars'), { tag, run });
        };
    };
    @property({
        type: Object
    ...
    _downloadDataUrl(_nameToDataSeries, dataSeriesName) {
        ...
        return tf_backend.addParams(tf_backend
            .getRouter()
            .pluginRoute('custom_scalars', '/download_data'), getVars);
    }
    _createProcessDataFunction(marginChartSeries) {
        ...
        return (scalarChart, run, data) => {
            ...
            _.forEach(marginChartSeries, (tagsObject) => {
                ...
                // tag of the value.
                const seriesName = tf_custom_scalar_dashboard.generateDataSeriesName(run, tagsObject.value);
    ...
    _createNewDataSeries(run, tag, seriesName, dataPoints) {
        ...
        const lineChartSymbol = vz_chart_helpers.SYMBOLS_LIST[this._runToNextAvailableSymbolIndex[run]];
        // Create a series with this name.
        const series = new tf_custom_scalar_dashboard.DataSeries(run, tag, seriesName, dataPoints, lineChartSymbol);
    ...
    get _tagFilter(): string {
        ...
        const tags = _.flatten(marginChartSeries.map((series) => [
            series.value,
            series.lower,
            series.upper,
        ]));
