...
export class DistributionChart {
    ...
    protected buildPlot(xAccessor, xScale, yScale): Plottable.Component {
        ...
        let opacities = _.range(percents.length - 1).map((i) => (percents[i + 1] - percents[i]) / 2500);
        ...
        let plots = _.range(accessors.length - 1).map((i) => {
            ...
            p.y(y, yScale);
            p.y0(y0);
            p.attr("fill", (d: any, i: number, dataset: Plottable.Dataset) => this.colorScale.scale(dataset.metadata().run));
            p.attr("stroke", (d: any, i: number, dataset: Plottable.Dataset) => this.colorScale.scale(dataset.metadata().run));
        ...
        medianPlot.attr("stroke", (d: any, i: number, m: any) => this.colorScale.scale(m.run));
        this.plots = plots;
...
class VzDistributionChart extends PolymerElement {
    static readonly template = html `<style include="plottable-style"></style>
    <div id="chartdiv"></div>
    <style>
      :host {
