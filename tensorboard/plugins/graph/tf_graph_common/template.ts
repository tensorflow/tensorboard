...
export function detect(
  ...
  return Object.keys(templates)
    .sort((key: string) => templates[key].level)
    .reduce(
      (obj, key) => {
...
interface SimilarSubgraphMeta {
  nodes: Node[];
  level: number;
}

type SimilarSubgraph = [string, SimilarSubgraphMeta];
...
function clusterSimilarSubgraphs(h: Hierarchy) {
  ...
  let hashDict = Object.keys(map).reduce(
    (reduced, name) => {
      ...
      let signature = getSignature(node as Metanode);
    ...
    {} as {
      [signature: string]: SimilarSubgraphMeta;
    }
  );
  return Object.keys(hashDict)
    .map((key) => [key, hashDict[key]] as SimilarSubgraph)
    .filter(([_, subGraph]) => {
    ...
    .sort(([_, subGraph]) => {
      ...
      const firstNode = subGraph.nodes[0];
      if (firstNode.type === NodeType.META) {
        return (firstNode as Metanode).depth;
      }
      return 0;
    });
}
...
function groupTemplateAndAssignId(
  ...
  return _.reduce(
    ...
    function(templates, nnGroupPair) {
      ...
      let nnGroup = nnGroupPair[1].nodes;
      let clusters: Array<{
        metanode: Metanode;
        members: string[];
      }> = [];
...
function sortNodes(
  names: string[],
  graph: Graph<Metanode | OpNode, Metaedge>,
  prefix: string
) {
...
function isSimilarSubgraph(g1: Graph<any, any>, g2: Graph<any, any>) {
  ...
  let g1prefix = g1.graph();
  let g2prefix = g2.graph();
  let visited1: {[name: string]: boolean} = {};
  let visited2: {[name: string]: boolean} = {};
  let stack: Array<{n1: string; n2: string}> = [];
  /**
