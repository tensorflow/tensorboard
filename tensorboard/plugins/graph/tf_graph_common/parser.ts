...
import {runAsyncPromiseTask} from './util';
import {GraphDef, StepStats} from './proto';
 ...
 */
export function parseGraphPbTxt(input: ArrayBuffer): Promise<GraphDef> {
  return parsePbtxtFile(input, GRAPH_REPEATED_FIELDS);
}
/**
 * Parses an ArrayBuffer of a proto txt file into a StepStats object.
 */
export function parseStatsPbTxt(input: ArrayBuffer): Promise<StepStats> {
...
function parsePbtxtFile(
  ...
  type AttributeValue = Object | string | number | boolean;
  ...
  function addAttribute(
    obj: {[name: string]: AttributeValue[] | AttributeValue},
    name: string,
