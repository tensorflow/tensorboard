...
import {Node, FUNCTION_LIBRARY_NODE_PREFIX, Metanode, MetaedgeImpl, createGraph, GraphType, Metaedge, GroupNode, OpNode, NodeType, getHierarchicalPath, OpNodeImpl, InclusionType, BridgeNode, EllipsisNode, createMetanode, ROOT_NAME, NAMESPACE_DELIM, EllipsisNodeImpl} from './graph';
import { Hierarchy } from './hierarchy';
import {Graph, GraphOptions} from './externs';
import { EdgeData, EDGE_WIDTH_SIZE_BASED_SCALE, MIN_EDGE_WIDTH, MAX_EDGE_WIDTH } from './edge';
import { hasDisplayableNodeStats } from './util';
...
export class RenderGraphInfo {
    ...
    computeScales() {
        this.deviceColorMap = d3
            ...
            .range(_.map(d3.range(this.hierarchy.devices!.length), MetanodeColors.DEVICE_PALETTE));
        this.xlaClusterColorMap = d3
            .scaleOrdinal<string>()
            .domain(this.hierarchy.xlaClusters!)
            .range(_.map(d3.range(this.hierarchy.xlaClusters!.length), MetanodeColors.XLA_CLUSTER_PALETTE));
    ...
    private colorHistogram(histogram: {
        [name: string]: number;
    }, colors: d3.ScaleOrdinal<string, string>): Array<{
        color: string;
        proportion: number;
    }> | null{
     ...
     */
    private cloneAndAddFunctionOpNode(parentMetanode: Metanode, libraryFunctionNodeName: string, node: OpNode, newPrefix: string): OpNode {
     ...
     *     library. This prefix should reflect graph hierarchy.
     */
    private cloneFunctionLibraryMetanode(metagraph: Graph<GroupNode | OpNode, Metaedge>, opNodeToReplace: OpNode, libraryMetanode: Metanode, oldPrefix: string, newPrefix: string): Metanode {
     ...
     */
    private cloneFunctionLibraryMetanodeHelper(metagraph: Graph<GroupNode | OpNode, Metaedge>, opNodeToReplace: OpNode, libraryMetanode: Metanode, oldPrefix: string, newPrefix: string, functionOutputIndexToNode: {
        [key: string]: Node;
    }): Metanode {
        ...
        newMetanode.xlaClusterHistogram = _.clone(libraryMetanode.xlaClusterHistogram);
        ...
        _.each(libraryMetanode.metagraph.nodes(), (nodeName) => {
            ...
            switch (node.type) {
                case NodeType.META:
                    ...
                    const newNode = this.cloneFunctionLibraryMetanodeHelper(metagraph, opNodeToReplace, node as Metanode, oldPrefix, newPrefix, functionOutputIndexToNode);
                ...
                case NodeType.OP:
                    ...
                    const newOpNode = this.cloneAndAddFunctionOpNode(newMetanode, oldPrefix, node as OpNode, newPrefix);
     ...
     */
    private cloneLibraryMetanodeEdges(libraryMetanode: Metanode, newMetanode: Metanode, oldPrefix: string, newPrefix: string) {
        _.each(libraryMetanode.metagraph.edges(), (edgeObject: graphlib.Edge) => {
            ...
            if (edge.baseEdgeList) {
                newMetaEdge.baseEdgeList = edge.baseEdgeList.map((baseEdge: graphlib.Edge) => {
     ...
     */
    private patchEdgesIntoFunctionInputs(opNodeToReplace: OpNode, newOpNode: OpNode) {
        ...
        let originalMetaEdge: Metaedge|null = null;
     ...
     */
    private patchEdgesFromFunctionOutputs(opNodeToReplace: OpNode, functionOutputIndexToDestinationNode: {
        [key: string]: Node;
    }) {
    ...
    buildSubhierarchy(nodeName: string): void {
        ...
        const nodesThatGotCloned: OpNode[] = [];
        const functionCallMetanodesToAdd: Metanode[] = [];
        ...
        let getBridgeNodeName = (inbound: boolean, ...rest: string[]) => rest.concat([inbound ? "IN" : "OUT"]).join("~~");
        ...
        _.each(bridgegraph.edges(), (bridgeEdgeObj) => {
            ...
            if (PARAMS.enableBridgegraph &&
                ...
                let findAdjoiningMetaedge = (targetName:string) => {
                    let adjoiningEdgeObj: graphlib.Edge = inbound
                        ? { v: targetName, w: nodeName }
                        : { v: nodeName, w: targetName };
                    return <RenderMetaedgeInfo>(parentNodeInfo.coreGraph.edge(adjoiningEdgeObj));
                };
     ...
     */
    private buildSubhierarchiesForNeededFunctions(metagraph: Graph<GroupNode | OpNode, Metaedge>) {
        _.each(metagraph.edges(), (edgeObj) => {
            let metaedge = metagraph.edge(edgeObj);
            let renderMetaedgeInfo = new RenderMetaedgeInfo(metaedge);
...
export class Annotation {
     ...
     *     out-annotation.
     */
    constructor(node: Node, renderNodeInfo: RenderNodeInfo, renderMetaedgeInfo: RenderMetaedgeInfo|null, type: AnnotationType, isIn: boolean) {
        this.node = node;
...
export class AnnotationList {
    ...
    push(annotation: Annotation): void {
      const nodeName = annotation.node.name;
        if (nodeName in this.nodeNames) {
            return; // Skip duplicate annotation.
        }
        this.nodeNames[nodeName] = true;
...
export class RenderNodeInfo {
     ...
     */
    isLibraryFunction: boolean = false;
    /**
    ...
    constructor(node: Node) {
        ...
        if (node.type === NodeType.META &&
            ...
            else if (_.startsWith(this.displayName, FUNCTION_LIBRARY_NODE_PREFIX)) {
...
function addInAnnotation(node: RenderNodeInfo, predecessor: Node, predecessorRenderInfo: RenderNodeInfo, edge: RenderMetaedgeInfo, type: AnnotationType): void {
    let annotation = new Annotation(predecessor, predecessorRenderInfo, edge, type, true);
    node.inAnnotations.push(annotation);
}
function addOutAnnotation(node: RenderNodeInfo, successor: Node, successorRenderInfo: RenderNodeInfo, edge: RenderMetaedgeInfo, type: AnnotationType): void {
    let annotation = new Annotation(successor, successorRenderInfo, edge, type, false);
    node.outAnnotations.push(annotation);
}
function setGraphDepth(graph: Graph<RenderNodeInfo, any>, depth: number) {
...
export class RenderGroupNodeInfo extends RenderNodeInfo {
     ...
     */
    coreGraph: Graph<RenderNodeInfo, RenderMetaedgeInfo>;
    ...
    constructor(groupNode: GroupNode, graphOptions: GraphOptions) {
        ...
        this.coreGraph = createGraph<RenderNodeInfo, RenderMetaedgeInfo>(gl||'', GraphType.CORE, graphOptions);
 ...
 * @param w Sink name.
 */
function createShortcut(graph: Graph<RenderNodeInfo, RenderMetaedgeInfo>, v: string, w: string) {
 ...
 */
function makeOutExtract(renderNode: RenderGroupNodeInfo, n: string, forceDetach?: boolean) {
 ...
 */
export function makeInExtract(renderNode: RenderGroupNodeInfo, n: string, forceDetach?: boolean) {
    let graph = renderNode.coreGraph;
    let child = graph.node(n);
...
function extractHighInOrOutDegree(renderNode: RenderGroupNodeInfo) {
    ...
    // nodes along the way.
    let nodeToInDegree: {[node: string]: number} = {};
    let nodeToOutDegree: {[node: string]: number} = {};
 ...
 */
export function expandUntilNodeIsShown(scene: TfGraphScene, renderHierarchy: RenderGraphInfo, tensorName: string) {
