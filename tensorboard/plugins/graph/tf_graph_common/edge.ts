...
import {BaseEdge, EDGE_KEY_DELIM, OpNode, Metaedge} from './graph';
...
export function getEdgeKey(edgeObj: {v: string; w: string}) {
...
export function buildGroup(
  ...
  graph: Graph<RenderNodeInfo, RenderMetaedgeInfo>,
  ...
  let container = selectOrCreateChild(sceneGroup, 'g', Class.Edge.CONTAINER);
...
export function appendEdge(
  edgeGroup: d3.Selection<SVGGElement, any, any, any>,
  d: {
    v: string;
    w: string;
    label?: RenderMetaedgeInfo;
  ...
  if (sceneElement.renderHierarchy.edgeWidthFunction) {
    ...
    strokeWidth = sceneElement.renderHierarchy.edgeWidthFunction(
      d as EdgeData,
      edgeClass
    );
  } else {
 ...
 */
function stylize(edgeGroup: d3.Selection<any, any, any, any>, d: EdgeData) {
  edgeGroup.classed('faded', d.label.isFadedOut);
  let metaedge = d.label.metaedge;
  edgeGroup
    .select('path.' + Class.Edge.LINE)
    .classed('control-dep', Boolean(metaedge && !metaedge.numRegularEdges));
}
