...
export interface Hierarchy {
  ...
  getBridgegraph(nodeName: string): Graph<GroupNode | OpNode, Metaedge> | null;
  ...
  getTemplateIndex(): (template: string) => number;
}
...
class HierarchyImpl implements Hierarchy {
  ...
  getBridgegraph(nodeName: string): Graph<GroupNode | OpNode, Metaedge> | null {
    ...
    [parentMetagraph, parentBridgegraph]
      .filter(Boolean)
      .forEach((parentGraph: Graph<any, any>) => {
        parentGraph
          .edges()
          ...
          .forEach((parentEdgeObj) => {
            ...
            _.each(parentMetaedge.baseEdgeList, (baseEdge) => {
              ...
              let bridgeEdgeObj = <graphlib.Edge>{
  ...
  getTopologicalOrdering(
    ...
    _.each(metagraph.edges(), (e: graphlib.Edge) => {
...
function findEdgeTargetsInGraph(
  graph: Graph<GroupNode | OpNode, Metaedge>,
  node: Node,
  inbound: boolean,
  targets: Edges
): void {
...
export function build(
  ...
  return runAsyncTask(
    ...
    () => {
      ...
      const deviceNames: {[device: string]: boolean} = {};
      const xlaClusterNames: {[xlaCluster: string]: boolean} = {};
    ...
    .then(() => {
      return runAsyncTask(
        ...
        30,
        () => {
          // type defined in `detect` is not compatible but it is yet unclear
          // what is supposed to be the truth.
          h.templates = detect(h, params.verifyTemplate) as INCORRECT_TYPING;
        },
        tracker
      );
    })
...
export function joinAndAggregateStats(h: Hierarchy, stats: StepStats) {
  ...
  _.each(h.root.leaves(), (nodeName) => {
    const leaf = <OpNode>h.node(nodeName);
    if (leaf.device != null) {
  ...
  _.each(h.root.leaves(), (nodeName) => {
    ...
    while (node.parentNode != null) {
      ...
      if (leaf.stats != null && node.parentNode.stats) {
...
function addNodes(h: Hierarchy, graph: SlimGraph) {
  ...
  const opToNode: {[op: string]: OpNode[]} = {};
...
function clusterNodes(
  metagraph: Graph<GroupNode | OpNode, Metaedge>
): {
  ...
  _.each(clusters, function(members, clusterId: string) {
    ...
    _.each(members, function(name: string) {
      ...
      while ((matchResult = numRegex.exec(leaf))) {
        ...
        forwardDict[seriesName].ids.push(Number(id));
