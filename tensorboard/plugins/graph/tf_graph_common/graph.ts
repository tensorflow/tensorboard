...
export interface OpNode extends Node {
  ...
  functionInputIndex: number | null;
  ...
  functionOutputIndex: number | null;
}
...
export interface GroupNode extends Node {
   ...
   * on demand as needed.
   */
  bridgegraph: Graph<any, any> | null;
  /**
...
export class OpNodeImpl implements OpNode {
  ...
  inputs: NormalizedInput[];
  type: NodeType.OP = NodeType.OP;
  isGroupNode: boolean = false;
  cardinality: number = 1;
  inEmbeddings: OpNode[] = [];
  outEmbeddings: OpNode[] = [];
  parentNode: Node | null = null;
  include: InclusionType = InclusionType.UNSPECIFIED;
  owningSeries: string | null = null;
  outputShapes: {
    [key: string]: TensorShape;
  };
  nodeAttributes: {
  ...
  // This field is only defined if the op node represents an input_arg to a
  // library function. It is the index of the input_arg.
  functionInputIndex: number | null = null;
  // This field is only defined if the op node represents an output_arg of a
  // library function. It is the index of the output_arg.
  functionOutputIndex: number | null = null;
  /**
...
export class NodeStats {
  ...
  constructor(outputSize: number[][] | null) {
   ...
   */
  addExecutionTime(startTime: number | null, endTime: number | null) {
...
export class MetanodeImpl implements Metanode {
  name: string;
  stats: NodeStats | null = null;
  type: NodeType.META = NodeType.META;
  depth: number;
  isGroupNode: boolean;
  cardinality: number;
  metagraph: Graph<any, any>;
  /** bridgegraph must be constructed lazily-see hierarchy.getBridgegraph() */
  bridgegraph: Graph<any, any> | null = null;
...
export interface Metaedge extends graphlib.Edge {
  /**
...
export class MetaedgeImpl implements Metaedge {
  ...
  baseEdgeList: BaseEdge[] = [];
  inbound?: boolean;
  numRegularEdges: number = 0;
  numControlEdges: number = 0;
  numRefEdges: number = 0;
  totalSize: number = 0;
...
function extractOutputShapes(
  ...
  for (let i = 0; i < attr.length; i++) {
    ...
    if (key === OUTPUT_SHAPES_KEY) {
      ...
      let result = value.list!.shape.reduce(
        (reduced, shape) => {
          let value: number[];
...
export function build(
  ...
  return runAsyncTask(
    ...
    () => {
      ...
      let index = 0;
      const processRawNode = (rawNode: NodeDef) => {
      ...
      const processFunction = (func: FunctionDef) => {
        ...
        if (func.signature.input_arg) {
          ...
          // property.
          if (!Array.isArray(func.signature.input_arg)) {
    ...
    return runAsyncTask(
      ...
      () => {
        ...
        _.each(opNodes, (opNode) => {
          const name = opNode.name;
          let normalizedName = normalizedNameDict[name] || name;
          graph.nodes[normalizedName] = opNode;
          // Check if the node has out-embeddings. If yes, add them to the
          // node.
          if (name in outEmbeddings) {
            opNode.outEmbeddings = outEmbeddings[name];
 ...
 */
function degreeSequence(graph: Graph<any, any>): number[] {
  let degrees = graph.nodes().map(function(name) {
    const neighbors = graph.neighbors(name);
...
export function hasSimilarDegreeSequence(
  graph1: Graph<any, any>,
  graph2: Graph<any, any>
): boolean {
