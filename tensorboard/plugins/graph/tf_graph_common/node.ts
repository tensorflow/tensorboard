...
import {TfGraphScene, ColorBy} from './tf-graph-scene';
import {
  RenderNodeInfo,
  RenderGraphInfo,
  RenderGroupNodeInfo,
...
import {getMenu, ContextMenuItem} from './contextmenu';
import {PARAMS, computeCXPositionOfNodeShape} from './layout';
import {escapeQuerySelector} from './util';
...
export function buildGroup(
  ...
  nodeData: RenderNodeInfo[],
  sceneElement: TfGraphScene
) {
  let container = selectOrCreateChild(sceneGroup, 'g', Class.Node.CONTAINER);
  ...
  let nodeGroups = (container as any)
    ...
    .data(nodeData, (d: RenderNodeInfo) => {
  ...
  nodeGroups
    .enter()
    .append('g')
    .attr('data-name', (d: RenderNodeInfo) => {
      return d.node.name;
    })
    ...
    .each(function(d: RenderNodeInfo) {
      ...
      // consistent.)
      let inAnnotationBox = selectOrCreateChild(
        nodeGroup,
        'g',
      ...
      // consistent.)
      let outAnnotationBox = selectOrCreateChild(
        nodeGroup,
        'g',
  ...
  nodeGroups
    .exit()
    .each(function(d: RenderGroupNodeInfo) {
...
function subsceneBuild(
  ...
  if (renderNodeInfo.node.isGroupNode) {
    if (renderNodeInfo.expanded) {
      ...
      return buildGroup(nodeGroup, [renderNodeInfo], sceneElement);
    }
 ...
 */
function subscenePosition(
  nodeGroup: d3.Selection<any, any, any, any>,
  d: RenderNodeInfo
) {
  let x0 = d.x - d.width / 2 + d.paddingLeft;
  let y0 = d.y - d.height / 2 + d.paddingTop;
  let subscene = selectChild(nodeGroup, 'g', Class.Subscene.GROUP);
...
function addButton(
  ...
  let group = selectOrCreateChild(selection, 'g', Class.Node.BUTTON_CONTAINER);
  selectOrCreateChild(group, 'circle', Class.Node.BUTTON_CIRCLE);
  selectOrCreateChild(group, 'path', Class.Node.EXPAND_BUTTON).attr(
    'd',
    'M0,-2.2 V2.2 M-2.2,0 H2.2'
  );
  selectOrCreateChild(group, 'path', Class.Node.COLLAPSE_BUTTON).attr(
    'd',
    'M-2.2,0 H2.2'
  );
...
function addInteraction(
  ...
  let contextMenuFunction = getMenu(
    sceneElement,
 ...
 */
export function getContextMenu(node: Node, sceneElement: TfGraphScene) {
...
function labelBuild(
  ...
  let label = selectOrCreateChild(nodeGroup, 'text', Class.Node.LABEL);
...
export function enforceLabelWidth(
  ...
  switch (nodeType) {
    case NodeType.META:
      if (renderNodeInfo && !renderNodeInfo.expanded) {
        // Only trim text if
        // node expanded.
        maxLength = PARAMS.nodeSize.meta.maxLabelWidth;
      }
      break;
    case NodeType.OP:
      maxLength = PARAMS.nodeSize.op.maxLabelWidth;
      break;
    case -1:
      maxLength = PARAMS.annotations.maxLabelWidth;
      break;
    default:
      break;
  }
  ...
  let baseText = labelContent.substr(0, index);
  do {
 ...
 */
let fontScale: d3.ScaleLinear<any, any> | null = null;
function getLabelFontScale(sceneElement: TfGraphScene) {
  if (!fontScale) {
    fontScale = d3
      .scaleLinear()
      .domain([
  ...
  switch (d.node.type) {
    case NodeType.OP:
      ...
      if (
        ...
        selectOrCreateChild(shapeGroup, 'polygon', Class.Node.COLOR_TARGET);
        break;
      }
      selectOrCreateChild(shapeGroup, 'ellipse', Class.Node.COLOR_TARGET);
      break;
    case NodeType.SERIES:
      // Choose the correct stamp to use to represent this series.
      let stampType = 'annotation';
      let groupNodeInfo = <RenderGroupNodeInfo>d;
      ...
      selectOrCreateChild(shapeGroup, 'use', classList).attr(
        'xlink:href',
        '#op-series-' + stampType + '-stamp'
      );
...
function position(
  ...
  let shapeGroup = selectChild(nodeGroup, 'g', Class.Node.SHAPE);
  ...
  switch (d.node.type) {
    case NodeType.OP: {
      ...
      if (
        ...
        // function.
        let shape = selectChild(shapeGroup, 'polygon');
        positionTriangle(shape, d.x, d.y, d.coreBox.width, d.coreBox.height);
      } else {
        let shape = selectChild(shapeGroup, 'ellipse');
    ...
    case NodeType.SERIES: {
      let shape = selectChild(shapeGroup, 'use');
      if (d.expanded) {
    ...
    case NodeType.BRIDGE: {
      ...
      let shape = selectChild(shapeGroup, 'rect');
      positionRect(shape, d.x, d.y, d.width, d.height);
      break;
    }
    default: {
...
export function getFillForNode(
  templateIndex: (id: string) => number,
  colorBy: ColorBy,
  renderInfo: RenderNodeInfo,
  isExpanded: boolean,
  svgRoot?: SVGElement
): string {
...
export function stylize(
  ...
  node.style('stroke', isSelected ? '' : getStrokeForFill(fillColor));
}
/**
...
function traceAllInputsOfOpNode(
  ...
  renderGraphInfo: RenderGraphInfo,
  startNode: OpNode,
  allTracedNodes: {[highlightedNodeName: string]: boolean}
) {
  ...
  let visibleInputs: {[name: string]: VisibleParent} = {};
  _.each(inputs, function(nodeInstance) {
    ...
    const resolvedOpNode = resolvedNode as OpNode;
    ...
    if (visibleInputsEntry) {
      visibleInputsEntry.opNodes.push(resolvedOpNode);
    } else {
      // Create new entry.
      visibleInputs[visibleParent.name] = {
        visibleParent: visibleParent,
        opNodes: [resolvedOpNode],
      };
    }
  });
  // Find all parents of the start node.
  let startNodeParents: StartNodeParents = {};
...
function _createVisibleTrace(
  svgRoot: SVGElement,
  nodeInstance: Node,
  startNodeParents: StartNodeParents,
