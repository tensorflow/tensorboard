...
import {RenderNodeInfo, RenderGroupNodeInfo} from './render';
import {PARAMS, MIN_AUX_WIDTH, computeCXPositionOfNodeShape} from './layout';
...
export function fit(
  svg: SVGSVGElement,
  zoomG: SVGGElement,
  d3zoom: d3.ZoomBehavior<any, any>,
  callback: () => void
) {
 ...
 *   <g class='core'>
 *     <g class='edges'>
 *       ... stuff from scene.edges.build ...
 *     </g>
 *     <g class='nodes'>
 *       ... stuff from scene.nodes.build ...
 *     </g>
 *   </g>
 *   <g class='in-extract'>
 *     <g class='nodes'>
 ...
 */
function position(
  sceneGroup: d3.Selection<any, any, any, any>,
  renderNode: RenderGroupNodeInfo
) {
  ...
  let yTranslate =
    renderNode.node.type === NodeType.SERIES
      ? 0
      : PARAMS.subscene.meta.labelHeight;
  // core
  ...
  if (hasLibraryFunctions) {
    let libraryFunctionsExtractX =
      renderNode.coreBox.width - renderNode.libraryFunctionsBox.width / 2;
    translate(
...
export function addGraphClickListener(
  graphGroup: SVGSVGElement,
  sceneElement: TfGraphScene
) {
...
export function translate(
  ...
  let selectionOrTransition:
    | d3.Selection<any, any, any, any>
    | d3.Transition<any, any, any, any> = selection;
  if (selection.attr('transform') != null) {
    selectionOrTransition = selection.transition('position');
  }
  selectionOrTransition.attr('transform', 'translate(' + x0 + ',' + y0 + ')');
}
/**
...
export function positionTriangle(
  polygon: d3.Selection<any, any, any, any>,
  cx: number,
  cy: number,
  width: number,
  height: number
...
export function positionButton(
  ...
  button
    ...
    .attr('cx', x)
    .attr('cy', y)
    .attr('r', PARAMS.nodeSize.meta.expandButtonRadius);
}
/**
...
export function positionEllipse(
  ellipse: d3.Selection<any, any, any, any>,
  cx: number,
  cy: number,
  width: number,
  height: number
 ...
 */
export function humanizeHealthPillStat(
  stat: number,
  shouldRoundOnesDigit: boolean
) {
...
export function addHealthPill(
  ...
  if (nodeGroupElement.parentElement) {
    nodeGroupElement.parentElement.appendChild(healthPillGroup);
  }
}
/**
...
export function addHealthPills(
  ...
  svgRootSelection
    ...
    .each(function(nodeInfo: RenderNodeInfo) {
