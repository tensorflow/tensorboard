...
export class Minimap {
  ...
  private download: HTMLLinkElement | null = null;
   ...
   * original svg.
   */
  private scaleMinimap: number | null = null;
  ...
  private translate: [number, number] | null = null;
  /** The last scaling factor used in the main svg. */
  private scaleMain: number | null = null;
  ...
  constructor(
    ...
    let $shadowRoot = d3.select((minimap.shadowRoot as unknown) as HTMLElement);
  ...
  private updateViewpoint(): void {
    ...
    // new viewpoint.
    const scaleMain = this.scaleMain!;
    const scaleMinimap = this.scaleMinimap!;
    let mainX = (-this.viewpointCoord.x * scaleMain) / scaleMinimap;
    let mainY = (-this.viewpointCoord.y * scaleMain) / scaleMinimap;
    d3.select(this.svg).call(
      this.mainZoom.transform,
      d3.zoomIdentity.translate(mainX, mainY).scale(scaleMain)
    );
  }
  /**
  ...
  update(): void {
    ...
    const downloadEl = $download.node() as HTMLLinkElement;
    this.download = downloadEl;
    $download.on('click', (d) => {
  ...
  zoom(transform?: d3.ZoomTransform): void {
    ...
    this.viewpointCoord.x = (-this.translate![0] * scaleMinimap) / scaleMain!;
    this.viewpointCoord.y = (-this.translate![1] * scaleMinimap) / scaleMain!;
    let viewpointWidth = (svgRect.width * scaleMinimap) / scaleMain!;
    let viewpointHeight = (svgRect.height * scaleMinimap) / scaleMain!;
    $viewpoint
      .attr('x', this.viewpointCoord.x)
      .attr('y', this.viewpointCoord.y)
