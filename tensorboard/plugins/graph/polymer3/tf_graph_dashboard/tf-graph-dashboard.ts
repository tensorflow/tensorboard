...
class TfGraphDashboard extends PolymerElement {
    static readonly template = html `<paper-dialog id="error-dialog" with-backdrop=""></paper-dialog>
    ...
    _fetchDataset() {
        return this._requestManager.request(tf_backend.getRouter().pluginRoute('graphs', '/info'));
    }
    /*
    ...
    _fetchHealthPills(nodeNames, step) {
        ...
        const url = tf_backend
            .getRouter()
            .pluginRoute('debugger', '/health_pills');
        return this._requestManager.request(url, postData);
    }
    _fetchDebuggerNumericsAlerts() {
        return this._requestManager.request(tf_backend.getRouter().pluginRoute('debugger', '/numerics_alert_report'));
    }
    _graphUrl(run, limitAttrSize, largeAttrsKey) {
        return tf_backend.getRouter().pluginRoute('graphs', '/graph', new URLSearchParams({
            run: run,
