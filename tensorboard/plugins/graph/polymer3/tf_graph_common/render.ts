...
export class RenderGraphInfo {
     ...
     *     library. This prefix should reflect graph hierarchy.
     */
    private cloneFunctionLibraryMetanode(metagraph: graphlib.Graph<GroupNode | OpNode, Metaedge>, opNodeToReplace: OpNode, libraryMetanode: Metanode, oldPrefix: string, newPrefix: string): Metanode {
     ...
     */
    private cloneFunctionLibraryMetanodeHelper(metagraph: graphlib.Graph<GroupNode | OpNode, Metaedge>, opNodeToReplace: OpNode, libraryMetanode: Metanode, oldPrefix: string, newPrefix: string, functionOutputIndexToNode: {
        [key: string]: Node;
    }): Metanode {
    ...
    private cloneLibraryMetanodeEdges(libraryMetanode: Metanode, newMetanode: Metanode, oldPrefix: string, newPrefix: string) {
        _.each(libraryMetanode.metagraph.edges(), (edgeObject: graphlib.EdgeObject) => {
     ...
     */
    private buildSubhierarchiesForNeededFunctions(metagraph: graphlib.Graph<GroupNode | OpNode, Metaedge>) {
        _.each(metagraph.edges(), (edgeObj) => {
            let metaedge = metagraph.edge(edgeObj);
            let renderMetaedgeInfo = new RenderMetaedgeInfo(metaedge);
...
function setGraphDepth(graph: graphlib.Graph<RenderNodeInfo, any>, depth: number) {
 ...
 * @param w Sink name.
 */
function createShortcut(graph: graphlib.Graph<RenderNodeInfo, RenderMetaedgeInfo>, v: string, w: string) {
 ...
 */
export function expandUntilNodeIsShown(scene, renderHierarchy, tensorName: string) {
