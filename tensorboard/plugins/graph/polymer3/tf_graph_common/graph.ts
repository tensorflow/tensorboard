 ...
 * graph information.
 */
export function joinStatsInfoWithGraph(graph: SlimGraph, stats: tf.graph.proto.StepStats, devicesForStats?: {
...
export class MetaedgeImpl implements Metaedge {
    ...
    private static computeSizeOfEdge(edge: BaseEdge, h: hierarchy.Hierarchy): number {
...
export function createSeriesNode(prefix: string, suffix: string, parent: string, clusterId: number, name: string, graphOptions: graphlib.GraphOptions): SeriesNode {
    return new SeriesNodeImpl(prefix, suffix, parent, clusterId, name, graphOptions);
}
export function getSeriesNodeName(prefix: string, suffix: string, parent: string, startId?: number, endId?: number): string {
...
class SeriesNodeImpl implements SeriesNode {
    ...
    constructor(prefix: string, suffix: string, parent: string, clusterId: number, name: string, graphOptions: graphlib.GraphOptions) {
...
function extractOutputShapes(attr: Array<{
    key: string;
    value: any;
}>): {
    [key: string]: TensorShape;
 ...
 */
// tslint:disable-next-line:no-any
function extractXlaCluster(attr: Array<{
    key: string;
    value: any;
}>): string | null {
    if (!attr) {
...
function addEdgeToGraph(graph: SlimGraph, inputName: string, outputNode: OpNode, input: NormalizedInput, params: BuildParams, index: number) {
...
export function build(graphDef: tf.graph.proto.GraphDef, params: BuildParams, tracker: ProgressTracker): Promise<SlimGraph> {
    /**
 ...
 */
export function createGraph<N, E>(name: string, type, opt?: graphlib.GraphOptions): graphlib.Graph<N, E> {
 ...
 *     new names.
 */
function mapStrictHierarchy(nodeNames: string[], embeddingNodeNames: string[]): {
 ...
 */
export function hasSimilarDegreeSequence(graph1: graphlib.Graph<any, any>, graph2: graphlib.Graph<any, any>): boolean {
 ...
 * ['a', 'a/b', 'a/b/c'].
 */
export function getHierarchicalPath(name: string, seriesNames?: {
    [name: string]: string;
}): string[] {
    let path: string[] = [];
 ...
 */
export function toggleNodeSeriesGroup(map: {
