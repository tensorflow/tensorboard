 ...
 */
export function selectOrCreateChild(container, tagName: string, className?: string | string[], before?): d3.Selection<any, any, any, any> {
 ...
 */
export function selectChild(container, tagName: string, className?: string | string[]): d3.Selection<any, any, any, any> {
 ...
 */
export function buildGroup(container, renderNode: render.RenderGroupNodeInfo, sceneElement: TfGraphScene, sceneClass: string): d3.Selection<any, any, any, any> {
    ...
    if (renderNode.isolatedOutExtract.length > 0) {
        ...
        node.buildGroup(outExtractGroup, renderNode.isolatedOutExtract, sceneElement);
    }
    else {
...
function position(sceneGroup, renderNode: render.RenderGroupNodeInfo) {
    ...
    if (hasInExtract) {
        ...
        if (auxWidth < layout.MIN_AUX_WIDTH) {
            inExtractX =
                inExtractX - layout.MIN_AUX_WIDTH + renderNode.inExtractBox.width / 2;
        }
        else {
            inExtractX =
                inExtractX -
 ...
 */
export function positionRect(rect, cx: number, cy: number, width: number, height: number) {
    rect
        .transition()
        .attr("x", cx - width / 2)
        .attr("y", cy - height / 2)
...
export function positionButton(button, renderNode: render.RenderNodeInfo) {
    ...
    button
        .select("circle")
        .transition()
        .attr({ cx: x, cy: y, r: layout.PARAMS.nodeSize.meta.expandButtonRadius });
}
/**
 ...
 * @param height Height to set.
 */
export function positionEllipse(ellipse, cx: number, cy: number, width: number, height: number) {
    ellipse
        .transition()
        .attr("cx", cx)
        .attr("cy", cy)
 ...
 */
function _getHealthPillTextContent(healthPill: HealthPill, totalCount: number, elementsBreakdown: number[], numericStats: HealthPillNumericStats) {
 ...
 */
export function addHealthPill(nodeGroupElement: SVGElement, healthPill: HealthPill, nodeInfo: render.RenderNodeInfo, healthPillId: number, healthPillWidth = 60, healthPillHeight = 10, healthPillYOffset = 0, textXOffset?: number) {
    ...
    if (Number.isFinite(numericStats.min) &&
        Number.isFinite(numericStats.max)) {
 ...
 */
export function addHealthPills(svgRoot: SVGElement, nodeNamesToHealthPills: {
    ...
    svgRootSelection
        ...
        const healthPill = healthPills
            ? healthPills[healthPillStepIndex]
            : null;
