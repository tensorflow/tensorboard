...
class HierarchyImpl implements Hierarchy {
    ...
    getBridgegraph(nodeName: string): graphlib.Graph<GroupNode | OpNode, Metaedge> {
        ...
        let bridgegraph = (groupNode.bridgegraph = createGraph<GroupNode | OpNode, Metaedge>("BRIDGEGRAPH", GraphType.BRIDGE, this.graphOptions));
 ...
 */
function findEdgeTargetsInGraph(graph: graphlib.Graph<GroupNode | OpNode, Metaedge>, node: Node, inbound: boolean, targets: Edges): void {
 ...
 */
export function build(graph: tf.graph.SlimGraph, params: HierarchyParams, tracker: ProgressTracker): Promise<Hierarchy> {
...
export function joinAndAggregateStats(h: Hierarchy, stats: tf.graph.proto.StepStats) {
    // Get all the possible device and XLA cluster names.
    let deviceNames = {};
    let xlaClusterNames = {};
...
export function getIncompatibleOps(hierarchy: Hierarchy, hierarchyParams: HierarchyParams) {
 ...
 */
function addEdges(h: Hierarchy, graph: SlimGraph, seriesNames: {
    [name: string]: string;
}) {
    ...
    _.each(graph.edges, (baseEdge) => {
        ...
        let sharedAncestorNode = <GroupNode>(nodeIndex[sourcePath[sourceAncestorIndex + 1]]);
 ...
 *     node.
 */
function groupSeries(metanode: Metanode, hierarchy: Hierarchy, seriesNames: {
    [name: string]: string;
}, threshold: number, map: {
    [name: string]: tf.graph.SeriesGroupingType;
}, useGeneralizedSeriesPatterns: boolean) {
    ...
    _.each(metagraph.nodes(), (n) => {
        ...
        if (child.type === tf.graph.NodeType.META) {
            groupSeries(<Metanode>child, hierarchy, seriesNames, threshold, map, useGeneralizedSeriesPatterns);
        }
    });
...
function clusterNodes(metagraph: graphlib.Graph<GroupNode | OpNode, Metaedge>): {
    ...
    return _.reduce(metagraph.nodes(), (clusters: {
        [clusterId: string]: string[];
    }, n: string) => {
        let child = metagraph.node(n);
 ...
 */
function detectSeriesUsingNumericSuffixes(clusters: {
    [clusterId: string]: string[];
}, metagraph: graphlib.Graph<GroupNode | OpNode, Metaedge>, graphOptions: graphlib.GraphOptions): {
    [seriesName: string]: SeriesNode;
} {
    let seriesDict: {
        [seriesName: string]: SeriesNode;
 ...
 * @return A dictionary from series name => seriesNode
 */
function detectSeriesAnywhereInNodeName(clusters: {
    [clusterId: string]: string[];
}, metagraph: graphlib.Graph<GroupNode | OpNode, Metaedge>, graphOptions: graphlib.GraphOptions): {
    [seriesName: string]: SeriesNode;
} {
 ...
 * @param graphOptions
 */
function addSeriesToDict(seriesNodes: SeriesNode[], seriesDict: {
    [seriesName: string]: SeriesNode;
}, clusterId: number, metagraph: graphlib.Graph<GroupNode | OpNode, Metaedge>, graphOptions: graphlib.GraphOptions) {
