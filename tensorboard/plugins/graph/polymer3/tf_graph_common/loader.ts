...
export function fetchAndConstructHierarchicalGraph(tracker: tf.graph.util.Tracker, remotePath: string | null, pbTxtFile: Blob | null, compatibilityProvider: op.CompatibilityProvider = new op.TpuCompatibilityProvider(), hierarchyParams: hierarchy.HierarchyParams = hierarchy.DefaultHierarchyParams): Promise<GraphAndHierarchy> {
    const dataTracker = util.getSubtaskTracker(tracker, 30, "Data");
    const graphTracker = util.getSubtaskTracker(tracker, 20, "Graph");
    const hierarchyTracker = util.getSubtaskTracker(tracker, 50, "Namespace hierarchy");
    return parser
