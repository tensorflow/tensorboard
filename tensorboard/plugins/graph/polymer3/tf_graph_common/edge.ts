 ...
 */
export function buildGroup(sceneGroup, graph: graphlib.Graph<render.RenderNodeInfo, render.RenderMetaedgeInfo>, sceneElement: TfGraphScene) {
 ...
 */
function getPathSegmentIndexAtLength(points: render.Point[], length: number, lineFunc: (points: render.Point[]) => string): number {
 ...
 */
function adjustPathPointsForMarker(points: render.Point[], marker: d3.Selection<any, any, any, any>, isStart: boolean): render.Point[] {
    let lineFunc = d3
        .line<render.Point>()
        .x((d) => d.x)
        .y((d) => d.y);
 ...
 */
export function appendEdge(edgeGroup, d: EdgeData, sceneElement: {
 ...
 */
function getEdgePathInterpolator(component: HTMLElement, renderPath: SVGPathElement, d: EdgeData, i: number, a: SVGPathElement[]) {
    ...
    if (d.label.startMarkerId) {
        points = adjustPathPointsForMarker(points, d3.select(shadowRoot.querySelector("#" + d.label.startMarkerId)), true);
    }
    if (d.label.endMarkerId) {
        points = adjustPathPointsForMarker(points, d3.select(shadowRoot.querySelector("#" + d.label.endMarkerId)), false);
    }
