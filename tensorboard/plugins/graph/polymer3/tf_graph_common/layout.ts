 ...
 */
function dagreLayout(graph: graphlib.Graph<render.RenderNodeInfo, render.RenderMetaedgeInfo>, params): {
    height: number;
    width: number;
} {
...
function layoutMetanode(renderNodeInfo: render.RenderGroupNodeInfo): void {
    ...
    // dimensions.
    _.extend(renderNodeInfo.coreBox, dagreLayout(renderNodeInfo.coreGraph, PARAMS.graph.meta));
    ...
    let maxInExtractWidth = renderNodeInfo.isolatedInExtract.length
        ? _.max(renderNodeInfo.isolatedInExtract, (renderNode) => renderNode.width).width
        : null;
    renderNodeInfo.inExtractBox.width =
        maxInExtractWidth != null ? maxInExtractWidth : 0;
    renderNodeInfo.inExtractBox.height = _.reduce(renderNodeInfo.isolatedInExtract, (height, child, i) => {
    ...
    let maxOutExtractWidth = renderNodeInfo.isolatedOutExtract.length
        ? _.max(renderNodeInfo.isolatedOutExtract, (renderNode) => renderNode.width).width
        : null;
    renderNodeInfo.outExtractBox.width =
        maxOutExtractWidth != null ? maxOutExtractWidth : 0;
    renderNodeInfo.outExtractBox.height = _.reduce(renderNodeInfo.isolatedOutExtract, (height, child, i) => {
        let yOffset = i > 0 ? params.extractYOffset : 0;
        // use width/height here to avoid overlaps between extracts
        child.x = 0;
        child.y = height + yOffset + child.height / 2;
    ...
    let maxLibraryFunctionsWidth = renderNodeInfo.libraryFunctionsExtract.length
        ? _.max(renderNodeInfo.libraryFunctionsExtract, (renderNode) => renderNode.width).width
        : null;
    renderNodeInfo.libraryFunctionsBox.width =
        maxLibraryFunctionsWidth != null ? maxLibraryFunctionsWidth : 0;
    renderNodeInfo.libraryFunctionsBox.height = _.reduce(renderNodeInfo.libraryFunctionsExtract, (height, child, i) => {
        let yOffset = i > 0 ? params.extractYOffset : 0;
        // use width/height here to avoid overlaps between extracts
        child.x = 0;
        child.y = height + yOffset + child.height / 2;
...
function layoutSeriesNode(node: render.RenderGroupNodeInfo): void {
    ...
    node.height =
        node.coreBox.height + params.paddingTop + params.paddingBottom;
}
/**
 ...
 */
function intersectPointAndNode(point: render.Point, node: render.RenderNodeInfo): render.Point {
