 ...
 */
export function fetchAndParseGraphData(path: string, pbTxtFile: Blob, tracker: ProgressTracker) {
    return tf.graph.util
 ...
 */
export function streamParse(arrayBuffer: ArrayBuffer, callback: (string) => void, chunkSize: number = 1000000, delim: string = "\n"): Promise<boolean> {
 ...
 */
export function parseGraphPbTxt(input: ArrayBuffer): Promise<tf.graph.proto.GraphDef> {
    return parsePbtxtFile(input, GRAPH_REPEATED_FIELDS);
}
/**
 * Parses an ArrayBuffer of a proto txt file into a StepStats object.
 */
export function parseStatsPbTxt(input: ArrayBuffer): Promise<tf.graph.proto.StepStats> {
 ...
 */
function parsePbtxtFile(input: ArrayBuffer, repeatedFields: {
    [attrPath: string]: boolean;
}): Promise<any> {
    let output: {
        [name: string]: any;
     ...
     */
    function addAttribute(obj: Object, name: string, value: Object | string | number | boolean, path: string[]): void {
