...
export function buildGroup(sceneGroup, nodeData: render.RenderNodeInfo[], sceneElement) {
    ...
    nodeGroups
        ...
        // consistent.)
        let inAnnotationBox = scene.selectOrCreateChild(nodeGroup, "g", Class.Annotation.INBOX);
        annotation.buildGroup(inAnnotationBox, d.inAnnotations, d, sceneElement);
        // Add g.out-annotations  (always add -- to keep layer order
        // consistent.)
        let outAnnotationBox = scene.selectOrCreateChild(nodeGroup, "g", Class.Annotation.OUTBOX);
        annotation.buildGroup(outAnnotationBox, d.outAnnotations, d, sceneElement);
...
function addButton(selection, d: render.RenderNodeInfo, sceneElement) {
    let group = scene.selectOrCreateChild(selection, "g", Class.Node.BUTTON_CONTAINER);
...
function addInteraction(selection, d: render.RenderNodeInfo, sceneElement: TfGraphScene, disableInteraction?: boolean) {
    ...
    let contextMenuFunction = contextmenu.getMenu(sceneElement, getContextMenu(d.node, sceneElement));
    selection
        .on("dblclick", (d) => {
        sceneElement.fire("node-toggle-expand", { name: d.node.name });
    })
...
export function getGroupSettingLabel(node: Node) {
    return tf.graph.getGroupSeriesNodeButtonString(getContainingSeries(node) !== null
 ...
 */
export function enforceLabelWidth(txtElementSelection: d3.Selection<any, any, any, any>, nodeType: NodeType | number, renderNodeInfo?: render.RenderNodeInfo): any {
...
function getGradient(id: string, colors: Array<{
    color: string;
    proportion: number;
 ...
 */
export function getFillForNode(templateIndex, colorBy, renderInfo: render.RenderNodeInfo, isExpanded: boolean, svgRoot?: SVGElement): string {
 ...
 */
export function updateInputTrace(svgRoot: SVGElement, renderGraphInfo: tf.graph.render.RenderGraphInfo, selectedNodeName: string, traceInputs: Boolean) {
    // Reset all styling.
 ...
 */
function _getAllContainedOpNodes(nodeName: string, renderGraphInfo: tf.graph.render.RenderGraphInfo): ReadonlyArray<OpNodeImpl> {
    let opNodes = [];
    // Get current node.
    let node = renderGraphInfo.getNodeByName(nodeName) as tf.graph.GroupNode | tf.graph.OpNode;
...
function traceAllInputsOfOpNode(svgRoot: SVGElement, renderGraphInfo: tf.graph.render.RenderGraphInfo, startNode: OpNode, allTracedNodes: Object) {
 ...
 * @private
 */
function _createVisibleTrace(svgRoot: SVGElement, nodeInstance: Node, startNodeParents, indexedStartNodeParents: Node[]) {
 ...
 * @private
 */
function _markParentsOfNodes(svgRoot: SVGElement, visibleNodes: {
    [nodeName: string]: Node;
}) {
 ...
 * @returns Node
 */
export function getVisibleParent(renderGraphInfo: tf.graph.render.RenderGraphInfo, currentNode: tf.graph.Node) {
    let found = false;
