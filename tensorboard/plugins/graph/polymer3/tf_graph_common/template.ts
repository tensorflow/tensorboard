...
limitations under the License.
==============================================================================*/
export function detect(h, verifyTemplate): {
...
function clusterSimilarSubgraphs(h: hierarchy.Hierarchy) {
    ...
    return Object.keys(hashDict)
        ...
        const node = nodes[0];
        return (node.type === NodeType.META && (node as Metanode).associatedFunction);
    })
        .sort(([_, subGraph]) => {
        // sort by depth
...
function sortNodes(names: string[], graph: graphlib.Graph<Metanode | OpNode, Metaedge>, prefix: string) {
