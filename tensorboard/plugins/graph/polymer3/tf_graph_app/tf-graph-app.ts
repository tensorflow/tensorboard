...
class TfGraphApp extends PolymerElement {
   ...
   to the left hand side. */
    ...
    <div class="container">
      <div class="all">
        <div class="side">
          <tf-graph-controls color-by-params="[[colorByParams]]" color-by="{{colorBy}}" render-hierarchy="[[_renderHierarchy]]" selected-node="{{selectedNode}}" selected-file="{{selectedFile}}" on-fit-tap="_fit" trace-inputs="{{_traceInputs}}"></tf-graph-controls>
          <tf-graph-loader id="loader" out-graph-hierarchy="{{graphHierarchy}}" out-graph="{{graph}}" progress="{{_progress}}" selected-file="[[selectedFile]]"></tf-graph-loader>
        </div>
        <div class="main">
          <tf-graph-board id="graphboard" graph-hierarchy="[[graphHierarchy]]" graph="[[graph]]" progress="[[_progress]]" color-by="[[colorBy]]" color-by-params="{{colorByParams}}" render-hierarchy="{{_renderHierarchy}}" selected-node="{{selectedNode}}" trace-inputs="[[_traceInputs]]"></tf-graph-board>
        </div>
      </div>
    </div>`;
    @property({
