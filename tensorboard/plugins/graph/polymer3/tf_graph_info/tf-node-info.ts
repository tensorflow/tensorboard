...
class TfNodeInfo extends PolymerElement {
    ...
    get _nodeStatsFormattedBytes(): string {
        ...
        return tf.graph.util.convertUnitsToHumanReadable(stats.totalBytes, tf.graph.util.MEMORY_UNITS);
    }
    @computed("_nodeStats")
    get _nodeStatsFormattedComputeTime(): string {
        ...
        return tf.graph.util.convertUnitsToHumanReadable(stats.getTotalMicros(), tf.graph.util.TIME_UNITS);
    }
    @computed("_nodeStats")
        ...
        var unpackMetaedge = (metaedge) => {
            return _.map(metaedge.baseEdgeList, (baseEdge) => {
                ...
                return {
                    ...
                    edgeLabel: tf.graph.scene.edge.getLabelForBaseEdge(baseEdge, this.renderHierarchy),
