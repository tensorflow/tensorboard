...
class RecordReader:
    ...
    def read(self):
        if self.filename is None:
            raise Exception('No filename provided, cannot read Events')
        if not os.path.exists(self.filename):
            raise Exception('Filename does not point to a valid Events file')
    ...
    def GetNext(self):
        ...
        if self.index >= len(self.event_strs) - 1:
            raise Exception('No more events to read')
        else:
            self.index += 1
