...
from . import compat, logging as tf_logging
...
class Registry(object):
    ...
    def register(self, candidate, name=None):
        ...
        if name in self._registry:
            (filename, line_number, function_name, _) = self._registry[name][
                _LOCATION_TAG
            ]
            raise KeyError(
        ...
        tf_logging.vlog(1, "Registering %s (%s) in %s.", name, candidate, self._name)
