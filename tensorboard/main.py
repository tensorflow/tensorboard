...
tf.flags.DEFINE_boolean(
    'purge_orphaned_data', True, 'Whether to purge data that '
                                 'may have been orphaned due to TensorBoard '
                                 'restarts. '
                                 'Disabling purge_orphaned_data can be used '
                                 'to debug data '
                                 'disappearance.')
...
def make_simple_server(tb_app, host, port):
    ...
    Returns:
        A tuple of (server, url): server: An HTTP server object configured to
        host TensorBoard. url: A best guess at a URL where TensorBoard will
        be accessible once the server has been started.

    Raises:
        socket.error: If a server could not be constructed with the host and
            port specified. Also logs an error message.
    """
    ...
    try:
        if host:
            ...
            # We've promised to bind to all interfaces on this host. However,
            #  we're not sure whether that means IPv4 or IPv6 interfaces.
            try:
                # First try passing in a blank host (meaning all interfaces).
                # This, unfortunately, defaults to IPv4 even if no IPv4
                # interface is available (yielding a socket.error).
                server = serving.make_server(host, port, tb_app, threaded=True)
            except socket.error:
                # If a blank host didn't work, we explicitly request IPv6
                # interfaces.
        ...
        if port == 0:
            ...
            msg = ('TensorBoard attempted to bind to port %d, but it was '
                   'already in use' % FLAGS.port)
