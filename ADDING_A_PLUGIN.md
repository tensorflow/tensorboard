...
You'll probably track multiple parameters in your model: say, the accuracy, precision, and cross-entropy. Each of these will have its own `tag`. Each `tag` tracks how a particular metric or measurement evolves over time. For each `run` there will be one or more `tag`.
...
We've discussed above why we need the `tag` (to differentiate data series) and the `tensor` (to hold the actual payload data). But why do we need metadata?
...
[scalar plugin backend]: https://github.com/tensorflow/tensorboard/blob/master/tensorboard/plugins/scalar/scalars_plugin.py
[histogram plugin backend]:https://github.com/tensorflow/tensorboard/blob/master/tensorboard/plugins/histogram/histograms_plugin.py
...
[scalar plugin demo]: https://github.com/tensorflow/tensorboard/blob/master/tensorboard/plugins/scalar/scalars_demo.py
[histogram plugin demo]: https://github.com/tensorflow/tensorboard/blob/master/tensorboard/plugins/histogram/histograms_demo.py

<!--
  - [x] The Tensor Summary op
  - [x] SummaryMetadata
  - [x] Examples: a Greeter, plus links to scalars and histograms
  - [x] Also - provide a non TF way to write plugin data. Describe API of the `plugins/$foo/summary.py`, which exposes `op` (TF) and `pb` (non-TF).
  - [x] *(nothing to say)* Tips & Tricks for consistent design with existing plugins
  - [x] Include a demo script that generates representative data that exercises all the features of your plugin, preferably following the API of existing such scripts.
  -->
...
To provide routes (endpoints) for your plugin, create a subclass of `BasePlugin`. Your subclass must have a static `plugin_name` property, and you'll need to implement the following methods:
  ...
  - `get_plugin_apps`: This should return a dictionary mapping routes to WSGI applications. More on this below.

[`TBContext`]: https://github.com/tensorflow/tensorboard/blob/master/tensorboard/plugins/base_plugin.py#L72
[event multiplexer]: https://github.com/tensorflow/tensorboard/blob/master/tensorboard/backend/event_processing/event_multiplexer.py
...
Below, `GreeterPlugin` demonstrates how a very basic plugin works. To peruse the code of a real plugin, see [`TextPlugin`](https://github.com/tensorflow/tensorboard/blob/efd5909b2ff60c2579b6052ed3c0e67402235623/tensorboard/plugins/text/text_plugin.py) or any of the plugins within TensorBoard's GitHub repository.
...
class GreeterPlugin(base_plugin.TBPlugin):
  ...
  # offered by this plugin, so it must be URL-friendly. This property
  # must uniquely identify this plugin from all other plugins.
  plugin_name = 'greeter'
  ...
  def tags_route(self, request):
    ...
    allRuns = self._multiplexer.PluginRunToTagToContent(
    ...
    response = {
        run: tagToContent.keys()
             for (run, tagToContent) in allRuns.items()
    }
  ...
  def is_active(self):
    ...
    # to the plugin.
    return bool(self._multiplexer and any(allRuns.values()))
...
<!--
  - Caveat: This is under active development. We'll help with migrations (create a GitHub issue)
  - TB loads the summary from disk / the logdir
  - Multiplexer is the way to get data out (for now). Example code.
  - You can also load arbitrary assets from disk; example code. Not encouraged.
  - Mention WSGI and Werkzeug (in passing)
...
[scalar dashboard code]:https://github.com/tensorflow/tensorboard/blob/master/tensorboard/plugins/scalar/tf_scalar_dashboard/tf-scalar-dashboard.html#L128
